...
class GenerationMixin:
    ...
    def _validate_model_kwargs(self, model_kwargs: Dict[str, Any]):
        """
        Validates `model_kwargs` for generation and checks for unused arguments.

        Args:
            model_kwargs (Dict[str, Any]): Keyword arguments to validate.
        
        Raises:
            ValueError: If unsupported or unused arguments are found.
        """
        # Validate `past_key_values` for cache compatibility
        if isinstance(model_kwargs.get("past_key_values"), Cache) and not self._supports_cache_class:
            raise ValueError(
                f"{self.__class__.__name__} does not support an instance of `Cache` as `past_key_values`. "
                "Check the model documentation for supported cache formats."
            )

        # Remove pre-processed arguments for encoder-decoder models
        if self.config.is_encoder_decoder:
            model_kwargs.pop("decoder_input_ids", None)

        # Collect all valid arguments for the model
        model_args = set(inspect.signature(self.prepare_inputs_for_generation).parameters)
        if "kwargs" in model_args or "model_kwargs" in model_args:
            model_args |= set(inspect.signature(self.forward).parameters)

        # Include encoder and decoder arguments for encoder-decoder models
        if self.config.is_encoder_decoder:
            base_model = getattr(self, self.base_model_prefix, None)

            # Collect encoder arguments
            encoder = getattr(self, "encoder", None) or getattr(base_model, "encoder", None)
            if encoder:
                model_args |= set(inspect.signature(encoder.forward).parameters)

            # Collect decoder arguments
            decoder = getattr(self, "decoder", None) or getattr(base_model, "decoder", None)
            if decoder:
                model_args |= {f"decoder_{arg}" for arg in inspect.signature(decoder.forward).parameters}

            # Include `assistant_encoder_outputs` if assisted generation is used
            if "assistant_encoder_outputs" in model_kwargs:
                model_args.add("assistant_encoder_outputs")

        # Check for unused arguments
        unused_model_args = [key for key in model_kwargs if key not in model_args]

        if unused_model_args:
            raise ValueError(
                f"The following `model_kwargs` are not used by the model: {unused_model_args}. "
                "Note: Typos in the generation arguments will also appear in this list."
            )
    ...
    def _prepare_generation_config(
        ...
        """
        Prepares the generation configuration, applying overrides from kwargs and ensuring compatibility.
        ...
        Returns:
            Tuple containing the updated generation configuration and model kwargs.
        """
        if generation_config is None:
            if (
                ...
                and self.config._get_non_default_generation_parameters()
            ):
                new_generation_config = GenerationConfig.from_model_config(self.config)
                if new_generation_config != self.generation_config:
                    warnings.warn(
                        "Modifying the pretrained model configuration for generation is deprecated and will be removed in v5. "
                        "Use and modify the model generation configuration instead. See: "
        ...
        if not is_torchdynamo_compiling():
            ...
            # Update generation_config with missing values from self.generation_config
            for attr in ["bos_token_id", "eos_token_id", "pad_token_id", "decoder_start_token_id"]:
                if getattr(generation_config, attr, None) is None:
                    setattr(generation_config, attr, getattr(self.generation_config, attr, None))
        else:
            model_kwargs = kwargs
    ...
    def _get_initial_cache_position(self, input_ids, model_kwargs): #torch.Size([1, 58]) torch.Size([1, 58]) torch.Size([2, 128, 3000]) torch.Size([2, 3000])
        """
        Calculates `cache_position` for the pre-fill stage based on `input_ids` 
        and optionally past length from `past_key_values`.
        """
        # Determine cache_position based on input type
        if "inputs_embeds" in model_kwargs and not self.config.is_encoder_decoder:
            cache_position = torch.arange(
                model_kwargs["inputs_embeds"].size(1), dtype=torch.int64, device=model_kwargs["inputs_embeds"].device
            )
        elif "decoder_inputs_embeds" in model_kwargs and self.config.is_encoder_decoder:
            cache_position = torch.arange(
                model_kwargs["decoder_inputs_embeds"].size(1), dtype=torch.int64, device=model_kwargs["decoder_inputs_embeds"].device
            )
        else:
            cache_position = torch.arange(
                input_ids.size(1), dtype=torch.int64, device=input_ids.device
            )

        # Adjust cache_position based on past length
        past_length = 0
        cache = model_kwargs.get("past_key_values")
        if cache is not None:
            if isinstance(cache, Cache) and hasattr(cache, "get_seq_length"):
                past_length = cache.get_seq_length() or 0
            elif isinstance(cache, list) and len(cache) > 0:
                past_length = cache[0][0].size(2)  # Assuming [layers, heads, seq_length, hidden_dim]

            # Adjust cache_position if not torch.compile-friendly
            if not is_torchdynamo_compiling() and past_length > 0:
    ...
    def _prepare_special_tokens(
        ...
        """
        Prepares the special tokens for generation and updates the generation config with processed tensors.

        Note: `generation_config` is modified in place and becomes non-serializable.
        """

        # Helper function to convert tokens to tensors
        def _to_tensor(token, device=None):
            if token is None:
                return None
            device = device or self.device
            return token if isinstance(token, torch.Tensor) else torch.tensor(token, device=device, dtype=torch.long)

        # Convert special tokens to tensors
        bos_token_tensor = _to_tensor(generation_config.bos_token_id, device)
        eos_token_tensor = _to_tensor(generation_config.eos_token_id, device)
        pad_token_tensor = _to_tensor(generation_config.pad_token_id, device)
        decoder_start_token_tensor = _to_tensor(generation_config.decoder_start_token_id, device)

        # Use `bos_token_id` as fallback for `decoder_start_token_id` in encoder-decoder models
        if self.config.is_encoder_decoder:
            decoder_start_token_tensor = decoder_start_token_tensor or bos_token_tensor

        # Ensure `eos_token_tensor` is 1D
        if eos_token_tensor is not None and eos_token_tensor.ndim == 0:
            eos_token_tensor = eos_token_tensor.unsqueeze(0)

        # Set `pad_token_tensor` if unset
        if pad_token_tensor is None and eos_token_tensor is not None:
            if not is_torchdynamo_compiling():
                if kwargs_has_attention_mask is False:
                    logger.warning(
                        "Attention mask and pad token id are not set. This may lead to unexpected behavior. "
                        "Pass an `attention_mask` for reliable results."
                    )
                logger.warning(f"Setting `pad_token_id` to `eos_token_id`: {eos_token_tensor[0]}")
            pad_token_tensor = eos_token_tensor[0]

        # Validate tokens
        if self.config.is_encoder_decoder and decoder_start_token_tensor is None:
            raise ValueError("`decoder_start_token_id` or `bos_token_id` must be defined for encoder-decoder generation.")
        if not is_torchdynamo_compiling():
            if (
                eos_token_tensor is not None
                and isin_mps_friendly(eos_token_tensor, pad_token_tensor).any()
            ):
                if kwargs_has_attention_mask is False:
                    logger.warning_once(
                        "Attention mask cannot be inferred because `pad_token_id` is the same as `eos_token_id`. "
                        "Pass an `attention_mask` for reliable results."
                    )
            if eos_token_tensor is not None and (
                ...
                logger.warning(
                    f"`eos_token_id` should be positive integers. Current value: {eos_token_tensor}. "
                    "This may lead to generation not stopping or crashes."
                )

        # Update generation config with tensors
    ...
    def _prepare_generation(self, generation_config,assistant_model, **kwargs):
        """
        Prepare and validate the model, tokenizer, and generation configuration.
        
        Args:
            generation_config: The configuration for generation.
            **kwargs: Additional arguments such as tokenizer and assistant-related configurations.

        Returns:
            generation_config: Updated generation configuration.
            model_kwargs: Keyword arguments for the model.
            tokenizer: The tokenizer to be used.
            assistant_tokenizer: Tokenizer for assisted generation, if applicable.
        """
        ...
        return generation_config, model_kwargs, tokenizer, assistant_tokenizer


    def _initialize_generation_parameters(self, logits_processor=None, stopping_criteria=None, synced_gpus=None, **model_kwargs):
        # Determine if GPUs are synced
        synced_gpus = synced_gpus or (
            (is_deepspeed_zero3_enabled() or is_fsdp_managed_module(self)) and dist.get_world_size() > 1
        )

        # Initialize default values for logits_processor and stopping_criteria
        logits_processor = logits_processor or LogitsProcessorList()
        stopping_criteria = stopping_criteria or StoppingCriteriaList()

        # Check attention mask usage
        accepts_attention_mask = "attention_mask" in inspect.signature(self.forward).parameters
        requires_attention_mask = "encoder_outputs" not in model_kwargs
        kwargs_has_attention_mask = model_kwargs.get("attention_mask", None) is not None


        return logits_processor, stopping_criteria, synced_gpus, accepts_attention_mask, requires_attention_mask, kwargs_has_attention_mask


    def _define_model_inputs(
        self,
        inputs,
        generation_config,
        model_kwargs,
        ...
        batch_size, device = inputs_tensor.shape[0], inputs_tensor.device # 1 device(type='cpu')

        # Prepare special tokens
        self._prepare_special_tokens(generation_config, kwargs_has_attention_mask, device)

        # Check padding for decoder-only models
        if (
            not self.config.is_encoder_decoder
            and not is_torchdynamo_compiling()
            and generation_config._pad_token_tensor is not None
            and batch_size > 1
            and inputs_tensor.ndim == 2
            and (inputs_tensor[:, -1] == generation_config._pad_token_tensor).any()
        ):
            logger.warning(
        ...
        return inputs_tensor, model_input_name, model_kwargs, batch_size, device


    def _define_other_model_kwargs(
        self,
        generation_config,
        model_kwargs,
        inputs_tensor,
        ...
        # Ensure caching for decoder-only models using `inputs_embeds`
        if not self.config.is_encoder_decoder and model_input_name == "inputs_embeds":
            generation_config.use_cache = True

        # Handle attention mask
        if requires_attention_mask and accepts_attention_mask:
            if not kwargs_has_attention_mask:
                model_kwargs["attention_mask"] = self._prepare_attention_mask_for_generation(
                    inputs_tensor, generation_config, model_kwargs
                )
            elif model_input_name == "input_ids" and model_kwargs["attention_mask"].ndim > 2:
                raise ValueError("`attention_mask` passed to `generate` must be 2D.")

        # Handle encoder-decoder specific kwargs
    ...
    def _prepare_input_ids_for_generation(
        ...
        # Prepare `input_ids` for encoder-decoder or decoder-only models
        if self.config.is_encoder_decoder:
        ...
        # Apply token healing if enabled
        if generation_config.token_healing and tokenizer is not None:
            input_ids = self.heal_tokens(input_ids, tokenizer)

        # Stream input IDs if a streamer is provided
    ...
    def _prepare_generation_lengths(
        self,
        generation_config,
        model_kwargs,
        inputs_tensor,
        ...
        **kwargs,
        ...
        # Check if default max_length and min_length are used
        has_default_max_length = kwargs.get("max_length") is None and generation_config.max_length is not None
        has_default_min_length = kwargs.get("min_length") is None and generation_config.min_length is not None

        # Prepare the generation configuration with updated lengths
    ...
    def _prepare_cache(
        ...
        # Retrieve user-defined cache if available
        user_defined_cache = model_kwargs.get(cache_name)

        # Set maximum cache length
        max_cache_length = generation_config.max_length

        # Adjust cache length for decoder-only models with `inputs_embeds`
        if (
        ...
        # Prepare the cache for generation
        self._prepare_cache_for_generation(
            generation_config, model_kwargs, assistant_model, batch_size, max_cache_length, device
        )

        return model_kwargs, user_defined_cache
    ...
    def _determine_generation_mode(
        ...
        # Validate streamer usage with beam search
        if streamer is not None and generation_config.num_beams > 1:
            raise ValueError(
                "`streamer` cannot be used with beam search (yet!). Make sure that `num_beams` is set to 1."
            )

        # Warn if `input_ids` and model devices differ
        if not is_torchdynamo_compiling() and self.device.type != input_ids.device.type:
            warnings.warn(
                "You are calling `.generate()` with the `input_ids` being on a device type different "
                f"from your model's device. `input_ids` is on {input_ids.device.type}, whereas the model "
                f"is on {self.device.type}. This may cause unexpected behavior or slower generation. "
                f"Please move `input_ids` to the correct device with `input_ids = input_ids.to('{self.device.type}')` "
    ...
    def _handle_assisted_generation(
        self,
        generation_config,
        batch_size,
        input_ids,
        ...
        if generation_config.num_return_sequences > 1:
            raise ValueError(
                "num_return_sequences must be 1 for assisted generation, "
                f"but is {generation_config.num_return_sequences}."
            )
        if batch_size > 1:
            raise ValueError("assisted generation only supports batch_size=1")
        if not model_kwargs["use_cache"]:
            raise ValueError("assisted generation requires `use_cache=True`")
        if generation_config.cache_implementation in ["static", "hybrid", "sliding_window"]:
            raise ValueError("assisted generation is not supported with static cache classes")
    ...
    def _handle_contrastive_search(
        self,
        generation_config,
        input_ids,
        prepared_logits_processor,
    ...
    def _handle_sample_or_greedy(
        ...
        # ->torch.Size([1, 58]) attention_mask:torch.Size([1, 58]) input_features:torch.Size([2, 128, 3000]) fearure_attention_mask:torch.Size([2, 3000])
        input_ids, model_kwargs = self._expand_inputs_for_generation(
            input_ids=input_ids, #torch.Size([1, 58])
    ...
    def generate(
        ...
        **kwargs,
        ...
        generation_config, model_kwargs, tokenizer, assistant_tokenizer = self._prepare_generation(generation_config, assistant_model, **kwargs)

        # 2. Set generation parameters if not already defined
        logits_processor, stopping_criteria, synced_gpus, accepts_attention_mask, requires_attention_mask, kwargs_has_attention_mask = self._initialize_generation_parameters(logits_processor=None, stopping_criteria=None, synced_gpus=None, **model_kwargs)

        # 3. Define model inputs
        inputs_tensor, model_input_name, model_kwargs, batch_size, device= self._define_model_inputs(inputs, generation_config, model_kwargs, kwargs_has_attention_mask)

        model_kwargs = self._define_other_model_kwargs(
            generation_config,
            model_kwargs,
            inputs_tensor,
            model_input_name,
        ...
        input_ids, model_kwargs = self._prepare_input_ids_for_generation(
            batch_size=batch_size,
            model_input_name=model_input_name,
            model_kwargs=model_kwargs,
            generation_config=generation_config,
        ...
        generation_config, model_kwargs = self._prepare_generation_lengths(
            generation_config,
            model_kwargs,
            inputs_tensor,
            model_input_name,
        ...
        # 7. Prepare the cache.
        model_kwargs, user_defined_cache = self._prepare_cache(
            generation_config=generation_config,
            model_kwargs=model_kwargs,
            assistant_model=assistant_model,
            batch_size=batch_size,
        ...
        generation_mode = self._determine_generation_mode( #生成模式是采样模式
            generation_config=generation_config,
            assistant_model=assistant_model,
            input_ids=input_ids,
            streamer=streamer,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            result = self._handle_assisted_generation(
                generation_config,
                batch_size,
                input_ids,
                inputs_tensor,
            ...
            result = self._handle_dola_generation(
                generation_config,
                input_ids,
                prepared_logits_processor,
                prepared_stopping_criteria,
            ...
            result = self._handle_contrastive_search(
                generation_config,
                input_ids,
                prepared_logits_processor,
                prepared_stopping_criteria,
            ...
            result = self._handle_sample_or_greedy(
                generation_config, #生成配置
                input_ids, #torch.Size([1, 58])
                prepared_logits_processor, #[类的集合]
                prepared_stopping_criteria, #[类的集合]
                synced_gpus, #false
                streamer, #none
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        """
        Sampling-based generation process.
        """
        # Initialize variables
        pad_token_id = generation_config._pad_token_tensor #tensor(151643)
        return_dict_in_generate = generation_config.return_dict_in_generate #false
        batch_size, cur_len = input_ids.shape # 1 58
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device) #tensor([1])
        this_peer_finished = False
        ...
        decoder_attentions = () if (return_dict_in_generate and generation_config.output_attentions) else None
        cross_attentions = () if (return_dict_in_generate and generation_config.output_attentions) else None
        decoder_hidden_states = () if (return_dict_in_generate and generation_config.output_hidden_states) else None
        encoder_attentions = encoder_hidden_states = None

        # Retrieve encoder outputs if the model is encoder-decoder
        if return_dict_in_generate and self.config.is_encoder_decoder:
            encoder_attentions = model_kwargs["encoder_outputs"].get("attentions") if generation_config.output_attentions else None
            encoder_hidden_states = model_kwargs["encoder_outputs"].get("hidden_states") if generation_config.output_hidden_states else None

        # Initialize cache position
        model_kwargs = self._get_initial_cache_position(input_ids, model_kwargs)

        while self._has_unfinished_sequences(
            this_peer_finished, synced_gpus, device=input_ids.device, cur_len=cur_len, max_length=generation_config.max_length
        ):
            # Prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            ...
            # Forward pass
            outputs = self(**model_inputs, return_dict=True) #最重点，数据前向传播
            ...
            if return_dict_in_generate:
                ...
                if generation_config.output_attentions:
                    decoder_attentions += (outputs.attentions,) if not self.config.is_encoder_decoder else (outputs.decoder_attentions,)
                    if self.config.is_encoder_decoder:
                        cross_attentions += (outputs.cross_attentions,)
                if generation_config.output_hidden_states:
                    decoder_hidden_states += (outputs.hidden_states,) if not self.config.is_encoder_decoder else (outputs.decoder_hidden_states,)
            ...
            # Update input_ids and unfinished_sequences
            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            if streamer is not None:
                streamer.put(next_tokens.cpu())
