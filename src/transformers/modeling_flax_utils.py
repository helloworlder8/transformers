        ...
        **kwargs,
        ...
        if model_name_or_path is not None:
            ...
            if os.path.isdir(model_name_or_path):
                if os.path.isfile(os.path.join(model_name_or_path, subfolder, FLAX_WEIGHTS_NAME)):
                    # Load from a Flax checkpoint
                    archive_file = os.path.join(model_name_or_path, subfolder, FLAX_WEIGHTS_NAME)
                elif os.path.isfile(os.path.join(model_name_or_path, subfolder, FLAX_WEIGHTS_INDEX_NAME)):
                    # Load from a sharded Flax checkpoint
                    archive_file = os.path.join(model_name_or_path, subfolder, FLAX_WEIGHTS_INDEX_NAME)
                    is_sharded = True
                ...
                elif from_pt and os.path.isfile(os.path.join(model_name_or_path, subfolder, WEIGHTS_NAME)):
                    # Load from a PyTorch checkpoint
                    archive_file = os.path.join(model_name_or_path, subfolder, WEIGHTS_NAME)
                elif from_pt and os.path.isfile(
                    os.path.join(model_name_or_path, subfolder, WEIGHTS_INDEX_NAME)
                ):
                    # Load from a sharded pytorch checkpoint
                    archive_file = os.path.join(model_name_or_path, subfolder, WEIGHTS_INDEX_NAME)
                    is_sharded = True
                ...
                elif os.path.isfile(os.path.join(model_name_or_path, subfolder, WEIGHTS_NAME)):
                    raise EnvironmentError(
                        f"Error no file named {FLAX_WEIGHTS_NAME} found in directory {model_name_or_path} "
                ...
                try:
                    ...
                    if resolved_archive_file is None and filename == FLAX_WEIGHTS_NAME:
                        resolved_archive_file = cached_file(
                            model_name_or_path, FLAX_WEIGHTS_INDEX_NAME, **cached_file_kwargs
                        )
                    ...
                    if resolved_archive_file is None:
                        ...
                        resolved_archive_file = cached_file(
                            model_name_or_path, SAFE_WEIGHTS_NAME, **cached_file_kwargs
                        )
                    ...
                    if resolved_archive_file is None:
                        ...
                        elif has_file(model_name_or_path, WEIGHTS_INDEX_NAME, **has_file_kwargs):
                            raise EnvironmentError(
                                f"{model_name_or_path} does not appear to have a file named"
        ...
        if missing_keys and not _do_init:
            logger.warning(
                f"The checkpoint {model_name_or_path} is missing required keys: {missing_keys}. "
        ...
        if len(fp16_params) > 0:
            logger.warning(
                ...
                f"the model checkpoint at {model_name_or_path}:\n{fp16_params}\n"
