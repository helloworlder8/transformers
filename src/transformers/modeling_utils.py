...
def _load_state_dict_into_meta_model(
    ...
    model_name_or_path=None,  # for flagging the user when the model contains renamed keys
):
    """
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin): #继承多个类
    r"""
    ...
    @staticmethod
    def _configure_device_map(device_map, low_cpu_mem_usage=None, ACCELERATE_MIN_VERSION="0.18.0"):
        ...
        if device_map is not None:
            if low_cpu_mem_usage is None:
                low_cpu_mem_usage = True #设置模式
            elif not low_cpu_mem_usage:
                raise ValueError("Passing along a `device_map` requires `low_cpu_mem_usage=True`")

        if low_cpu_mem_usage: #处理不兼容问题
            if is_deepspeed_zero3_enabled():
                raise ValueError(
    ...
    def _handle_quantization_config(load_in_4bit, load_in_8bit, quantization_config=None, **kwargs):
        if load_in_4bit or load_in_8bit:
            ...
            logger.warning(
                "The `load_in_4bit` and `load_in_8bit` arguments are deprecated and will be removed in future versions. "
                "Please, pass a `BitsAndBytesConfig` object in the `quantization_config` argument instead."
            )

        return quantization_config, kwargs

    @staticmethod
    def _get_resolved_archive_file(
        ...
        if model_name_or_path is not None and gguf_file is None:
            ...
            if is_local:
                if from_tf and os.path.isfile(
                    ...
                    os.path.join(model_name_or_path, subfolder, TF2_WEIGHTS_NAME)
                ):
    ...
    @staticmethod
    def _set_model_dtype(cls, config, state_dict, resolved_archive_file, is_sharded, weights_only, sharded_metadata, torch_dtype,hf_quantizer,gguf_path,low_cpu_mem_usage, model_name_or_path):
    ...
    @staticmethod
    def prepare_device_map_and_quantizer(
        model, 
        config,
        device_map,
        low_cpu_mem_usage, 
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if gguf_file is not None and hf_quantizer is not None: #不兼容问题
            raise ValueError(
                "You cannot combine Quantization and loading a model from a GGUF file, try again by making sure you did not passed a `quantization_config` or that you did not load a quantized model from the Hub."
            )
            
        """ 获取归档文件 """
        resolved_archive_file, is_sharded = cls._get_resolved_archive_file(model_name_or_path, #归档文件和碎片化
            gguf_file,
            from_tf,
            from_flax,
            use_safetensors,
        ...
        if model.can_generate() and generation_config is not None:
            ...
            try:
                model.generation_config = GenerationConfig.from_pretrained( #生成配置类
                    model_name_or_path,
                    cache_dir=cache_dir,
                    force_download=force_download,
                    resume_download=resume_download,
        ...
        """ 后处理 """
        if device_map is not None:
            device_map_kwargs = cls._dispatch_model(        
                device_map,
                offload_folder,
                offload_index,
                offload_buffers,
