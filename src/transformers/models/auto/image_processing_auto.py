...
class AutoImageProcessor:
    ...
    def __init__(self):
        raise EnvironmentError(
            ...
            "using the `AutoImageProcessor.from_pretrained(model_name_or_path)` method."
        )
    ...
    def from_pretrained(cls, model_name_or_path, *inputs, **kwargs):
        ...
        (either passed as an argument or loaded from `model_name_or_path` if possible), or when it's
        missing, by falling back to using pattern matching on `model_name_or_path`:
        ...
        if has_remote_code and trust_remote_code:
            ...
            image_processor_class = get_class_from_dynamic_module(class_ref, model_name_or_path, **kwargs)
        ...
        # Last try: we use the IMAGE_PROCESSOR_MAPPING.
            ...
            if image_processor_class_fast and (use_fast or image_processor_class_py is None):
                return image_processor_class_fast.from_pretrained(model_name_or_path, *inputs, **kwargs)
            else:
                if image_processor_class_py is not None:
                    return image_processor_class_py.from_pretrained(model_name_or_path, *inputs, **kwargs)
                else:
                    raise ValueError(
        ...
        raise ValueError(
            f"Unrecognized image processor in {model_name_or_path}. Should have a "
