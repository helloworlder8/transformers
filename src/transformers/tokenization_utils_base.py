...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def _determine_batched_conversations(
        self, 
        conversation: Union[List[Dict[str, str]], List[List[Dict[str, str]]]]
    ) -> Tuple[List[List[Dict[str, str]]], bool]:

        is_batched = isinstance(conversation, (list, tuple)) and (
    ...
    def _process_tools(self, tools: Optional[List[Union[Dict]]]) -> Optional[List[Dict]]:
        if not tools:
            return None
        ...
        for tool in tools:
            if isinstance(tool, dict):
                ...
                raise ValueError(
                    "Each tool must be a JSON schema (dict) or a callable function with type hints "
                    "and a docstring suitable for conversion to a schema."
                )
        return tool_schemas
    ...
    def _render_conversations(
        self,
        conversations: List[Union[List[Dict[str, str]], Any]],
        ...
        **kwargs,
        ...
        template_kwargs = {**self.special_tokens_map, **kwargs}  # Merge special tokens and additional arguments
        rendered = []
        all_generation_indices = []

        for chat in conversations:
            # Extract messages if chat is a Conversation object
            ...
            # Handle continuation of the final message if needed
            if continue_final_message:
                rendered_chat = self._handle_continue_final_message(messages, rendered_chat)
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        conversations, is_batched = self._determine_batched_conversations(conversation)
        chat_template = self.get_chat_template(chat_template, tools)
        compiled_template = _compile_jinja_template(chat_template)


        tool_schemas = self._process_tools(tools)


        rendered, all_generation_indices = self._render_conversations(
            conversations,
            compiled_template,
            tool_schemas,
            documents,
    ...
    def get_chat_template(self, chat_template: Optional[str] = None, tools: Optional[List[Dict]] = None) -> str:
        ...
        Args:
            chat_template (`str`, *optional*): The name or content of a template to use.
            tools (`List[Dict]`, *optional*): List of tools as JSON Schema for tool-supported templates.

        Returns:
            `str`: The chat template string.
        """
        # If multiple templates are available (as a dictionary)
        if isinstance(self.chat_template, dict):
            template_dict = self.chat_template
            if chat_template:
                # Use the specified template if available
                chat_template = template_dict.get(chat_template)
                if not chat_template:
                    raise ValueError(f"Template '{chat_template}' not found! Available templates: {sorted(template_dict.keys())}")
            else:
                # Fallback to tool-use template or default template
                chat_template = template_dict.get("tool_use") if tools else template_dict.get("default")
                if not chat_template:
                    raise ValueError(
                        "No default chat template specified! Please pass a template name or set a default in "
                        f"the template dictionary. Available templates: {sorted(template_dict.keys())}"
                    )
        elif chat_template is None:
            # For single-template cases, use the tokenizer's chat_template or raise an error
            chat_template = self.chat_template
            if not chat_template:
                raise ValueError(
                    "No chat template is set for the tokenizer and no argument was passed! "
                    "See documentation: https://huggingface.co/docs/transformers/main/en/chat_templating"
                )
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        single_file_id = None
        if os.path.isfile(model_name_or_path) or is_remote_url(model_name_or_path):
            ...
            vocab_files[file_id] = model_name_or_path
            single_file_id = file_id
        else:
            if gguf_file:
                vocab_files["vocab_file"] = gguf_file
            else:
                # At this point model_name_or_path is either a directory or a model identifier name
        ...
        if all(full_file_name is None for full_file_name in resolved_vocab_files.values()) and not gguf_file:
            raise EnvironmentError(
                f"Can't load tokenizer for '{model_name_or_path}'. If you were trying to load it from "
                "'https://huggingface.co/models', make sure you don't have a local directory with the same name. "
                f"Otherwise, make sure '{model_name_or_path}' is the correct path to a directory "
    ...
    def _from_pretrained(
        ...
        **kwargs,
            ...
            if config_tokenizer_class is None:
                ...
                if hasattr(config, "model_type"):
                    ...
                    for pattern in TOKENIZER_MAPPING_NAMES.keys():
                        if pattern in str(model_name_or_path):
                            model_type = pattern
                            break
        ...
        init_kwargs["name_or_path"] = model_name_or_path
    ...
    @staticmethod
    def _eventually_correct_t5_max_length(model_name_or_path, max_model_length, init_max_model_length):
    ...
    def _get_padding_truncation_strategies(
        ...
        """
        Determine the correct padding and truncation strategies, with backward compatibility.
        """
        # Handle deprecated arguments
        old_truncation_strategy = kwargs.pop("truncation_strategy", "do_not_truncate")
        old_pad_to_max_length = kwargs.pop("pad_to_max_length", False)

        # Default truncation strategy if max_length is provided without truncation
        if max_length is not None and not padding and truncation is None:
            if verbose and not self.deprecation_warnings.get("Truncation-not-explicitly-activated", False):
        ...
        if padding is False and old_pad_to_max_length:
            padding_strategy = (
                PaddingStrategy.MAX_LENGTH if max_length is not None else PaddingStrategy.LONGEST
            )
            ...
            padding_strategy = PaddingStrategy(padding or "do_not_pad")

        # Determine truncation strategy
        if truncation is None and old_truncation_strategy != "do_not_truncate":
            truncation_strategy = TruncationStrategy(old_truncation_strategy)
        elif truncation is True:
            truncation_strategy = TruncationStrategy.LONGEST_FIRST
        elif isinstance(truncation, TruncationStrategy):
            truncation_strategy = truncation
        else:
            truncation_strategy = TruncationStrategy(truncation or "do_not_truncate")

        # Set max length if needed
        if max_length is None:
            if padding_strategy == PaddingStrategy.MAX_LENGTH or truncation_strategy != TruncationStrategy.DO_NOT_TRUNCATE:
                if self.model_max_length > LARGE_INTEGER:
                    if verbose and not self.deprecation_warnings.get("Max-length-warning", False):
                        logger.warning(
                            "Model has no predefined max length. Defaulting to no truncation or padding."
                        )
                        self.deprecation_warnings["Max-length-warning"] = True
        ...
        if padding_strategy != PaddingStrategy.DO_NOT_PAD and (self.pad_token is None or self.pad_token_id < 0):
            raise ValueError(
                "Padding is requested, but the tokenizer does not have a padding token. Set `pad_token` or use "
                "`tokenizer.add_special_tokens({'pad_token': '[PAD]'})`."
            )

        # Ensure truncation and padding align with `pad_to_multiple_of`
        if (
            truncation_strategy != TruncationStrategy.DO_NOT_TRUNCATE
            ...
            raise ValueError(
                f"Truncation length ({max_length}) must be a multiple of `pad_to_multiple_of` ({pad_to_multiple_of})."
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Collect all arguments for consistency
        all_kwargs = {
            "add_special_tokens": add_special_tokens,
            "padding": padding,
            "truncation": truncation,
    ...
    def _call_one(
        ...
        **kwargs,
        ...
        def _is_valid_text_input(t) -> bool:
            """Validate that the input is a valid text format."""
            if isinstance(t, str):
                return True
            if isinstance(t, (list, tuple)):
                return all(isinstance(sub, (str, list, tuple)) for sub in t)
            return False

        if not _is_valid_text_input(text):
            raise ValueError("Invalid `text` input. Must be `str`, `List[str]`, or `List[List[str]]`.")
        if text_pair is not None and not _is_valid_text_input(text_pair):
            raise ValueError("Invalid `text_pair` input. Must be `str`, `List[str]`, or `List[List[str]]`.")

        is_batched = isinstance(text, (list, tuple)) and (
            is_split_into_words and text and isinstance(text[0], (list, tuple))
            or not is_split_into_words
        )

        if is_batched:
            if isinstance(text_pair, str):
                raise TypeError("For batched inputs, `text_pair` must be a list with the same length as `text`.")
            if text_pair is not None and len(text) != len(text_pair):
                raise ValueError(f"Batch size mismatch: `text` has length {len(text)} while `text_pair` has length {len(text_pair)}.")

            batch_text_or_text_pairs = list(zip(text, text_pair)) if text_pair is not None else text
            return self.batch_encode_plus(
                batch_text_or_text_pairs=batch_text_or_text_pairs, #['chair . person . dog .']
                ...
                return_overflowing_tokens=return_overflowing_tokens,
                return_special_tokens_mask=return_special_tokens_mask,
                return_offsets_mapping=return_offsets_mapping, #false
    ...
    def encode_plus(
        ...
        **kwargs,
        ...
        return self._encode_plus(
            ...
            pad_to_multiple_of=pad_to_multiple_of, #None
            padding_side=padding_side,#None
            return_tensors=return_tensors,#None
            return_token_type_ids=return_token_type_ids,#None
            return_attention_mask=return_attention_mask,#None
