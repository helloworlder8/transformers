...
def main():
    ...
    ds = load_dataset(
        data_args.dataset_name, #'cifar10'
        data_args.dataset_config_name, #None
        data_files=data_args.data_files,#None
    ...
    if model_args.config_name_or_path: #None
    ...
    # adapt config
    model_args.image_size = model_args.image_size if model_args.image_size is not None else config.image_size #有模型参数用模型参数，没有用配置参数
    ...
    config.update(
        {
            "image_size": model_args.image_size, #模型本来就有的话模型是老大
    ...
    if training_args.do_train:
        column_names = ds["train"].column_names #['img', 'label']
    else:
    ...
    if training_args.do_train:
        ...
        train_result = trainer.train(resume_from_checkpoint=checkpoint) #最重点，正式开始训练了
        trainer.save_model()
