...
class PretrainedConfig(PushToHubMixin):
    ...
    Arg:
        name_or_path (`str`, *optional*, defaults to `""`):
            ...
            [`TFPreTrainedModel.from_pretrained`] as `model_name_or_path` if the configuration was created
            with such a method.
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        config_dict, kwargs = cls.get_config_dict(model_name_or_path, **kwargs)
    ...
    def get_config_dict(
        ...
        config_dict, kwargs = cls._get_config_dict(model_name_or_path, **kwargs)
        ...
        if "configuration_files" in config_dict:
            ...
            config_dict, kwargs = cls._get_config_dict(
                model_name_or_path, _configuration_file=configuration_file, **original_kwargs
            )
    ...
    def _get_config_dict(
        ...
        if os.path.isfile(os.path.join(subfolder, model_name_or_path)):
            # Special case when model_name_or_path is a local file
            resolved_config_file = model_name_or_path
            is_local = True
        elif is_remote_url(model_name_or_path):
            configuration_file = model_name_or_path if gguf_file is None else gguf_file
            resolved_config_file = download_url(model_name_or_path)
        else:
            configuration_file = kwargs.pop("_configuration_file", CONFIG_NAME) if gguf_file is None else gguf_file

            try:
                # Load from local folder or from cache or download from model Hub and cache
                resolved_config_file = cached_file( #'ByteDance/Sa2VA-1B/config.json'
                    model_name_or_path,
                    configuration_file,
                    cache_dir=cache_dir,
                ...
                if resolved_config_file is None:
                    return None, kwargs
                commit_hash = extract_commit_hash(resolved_config_file, commit_hash) #none
                ...
                raise EnvironmentError(
                    f"Can't load the configuration of '{model_name_or_path}'. If you were trying to load it"
                    " from 'https://huggingface.co/models', make sure you don't have a local directory with the same"
                    f" name. Otherwise, make sure '{model_name_or_path}' is the correct path to a directory"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Returns:
            `Dict[str, Any]`: Dictionary of all the attributes that make up this configuration instance.
        """
        output = copy.deepcopy(self.__dict__) #获取所有的字典
