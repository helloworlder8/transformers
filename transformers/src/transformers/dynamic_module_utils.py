...
def get_cached_module_file(
    ...
    **deprecated_kwargs,
    ...
    # Download and cache module_file from the repo `model_name_or_path` of grab it if it's a local file.
    model_name_or_path = str(model_name_or_path)
    is_local = os.path.isdir(model_name_or_path)
    if is_local:
        submodule = os.path.basename(model_name_or_path)
    else:
        submodule = model_name_or_path.replace("/", os.path.sep)
        cached_module = try_to_load_from_cache(
            model_name_or_path, module_file, cache_dir=cache_dir, revision=_commit_hash, repo_type=repo_type
        )
    ...
    try:
        ...
        logger.error(f"Could not locate the {module_file} inside {model_name_or_path}.")
        raise
    ...
    if submodule == os.path.basename(model_name_or_path):
        ...
        for module_needed in modules_needed:
            ...
            module_needed_file = os.path.join(model_name_or_path, module_needed)
    ...
    if len(new_files) > 0 and revision is None:
        ...
        url = f"https://huggingface.co/{repo_type_str}{model_name_or_path}"
        logger.warning(
