...
class GenerationMixin:
    ...
    def _prepare_generation(self, generation_config,use_model_defaults, assistant_model, **kwargs):
        ...
        return generation_config, tokenizer, assistant_tokenizer, model_kwargs


    def _initialize_generation_parameters(self, synced_gpus=None, logits_processor=None, stopping_criteria=None, **model_kwargs):
        ...
        return synced_gpus, logits_processor, stopping_criteria, accepts_attention_mask, requires_attention_mask, kwargs_has_attention_mask
    ...
    def _define_model_inputs(
        ...
        ) #-》torch.Size([1, 18]) 'input_ids' {'attention_mask': torch.Size([1, 18])}
    ...
    def _prepare_input_ids_for_generation(
        self,
        generation_config, #class
        inputs_tensor, #torch.Size([1, 590, 768])
        model_input_name, #'inputs_embeds'

        model_kwargs, #{inputs_embeds, attention_mask, encoder_outputs}
        ...
        if self.config.is_encoder_decoder:
            input_ids, model_kwargs = self._prepare_decoder_input_ids_for_generation(
                ...
                model_input_name=model_input_name, #'inputs_embeds'
                model_kwargs=model_kwargs,
                decoder_start_token_id=generation_config._decoder_start_token_tensor, #torch.Size([]) tensor(2, device='cuda:0')
    ...
    def _handle_greedy_sample(
        ...
        input_ids, model_kwargs = self._expand_inputs_for_generation( #->torch.Size([1, 18]) {'attention_mask'}
            input_ids=input_ids, #torch.Size([1, 18])
            expand_size=generation_config.num_return_sequences, #1
            is_encoder_decoder=self.config.is_encoder_decoder, #false
            **model_kwargs, #{'attention_mask': torch.Size([1, 18]), 'logits_to_keep': 1, 'past_key_values': <transformers.cache_utils.DynamicCache object at 0x7fa28b834130>, 'use_cache': True}
        )

        # 执行采样或贪心搜索
        return self._sample(
            generation_config=generation_config, #class
            input_ids=input_ids, #torch.Size([1, 18])
            logits_processor=prepared_logits_processor, #list
            stopping_criteria=prepared_stopping_criteria, #list

            synced_gpus=synced_gpus, #false
            streamer=streamer, #none
            **model_kwargs, #{}
        )

    def _handle_group_beam_search(
        self,
        generation_config, #class
        input_ids, #torch.Size([1, 1]) tensor([[2]], device='cuda:0')
        inputs_tensor, #torch.Size([1, 590, 768])
        prepared_logits_processor, #[class,class, ...]
        prepared_stopping_criteria, #[class,class, ...]
        synced_gpus, #false
        model_kwargs, #{'inputs_embeds':_,}
    ):
        beam_scorer = BeamSearchScorer( #束搜索
        ...
        input_ids, model_kwargs = self._expand_inputs_for_generation( #-》torch.Size([3, 1]) input_embeds attention_mask encoder_outputs use_cache=true
        ...
        result = self._beam_search(
            input_ids, #torch.Size([3, 1])
            beam_scorer, #class
            logits_processor=prepared_logits_processor, #[class,class]
            stopping_criteria=prepared_stopping_criteria,#[class,class]
            generation_config=generation_config, #class
            synced_gpus=synced_gpus, #false
            **model_kwargs, #input_embeds attention_mask encoder_outputs use_cache=true
        )
    ...
    def _handle_constrained_beam_search(
        self,
        input_ids,
        inputs_tensor,
        model_kwargs,
        ...
        """
        执行带有 force_words_ids 的 Constrained Beam Search 推理流程。
        """
        ...
        if generation_config.force_words_ids is not None:
            def typeerror():
                raise ValueError(
                    "`force_words_ids` must be List[List[int]] or List[List[List[int]]], "
                    f"got: {generation_config.force_words_ids}"
                )

            for word_ids in generation_config.force_words_ids:
                if isinstance(word_ids[0], list):  # List[List[int]]
                    if not all(isinstance(token_ids, list) for token_ids in word_ids):
                        typeerror()
                    if any(
    ...
    def generate(
        ...
        **kwargs, #{'input_ids': torch.Size([1, 18]), 'attention_mask': torch.Size([1, 18]), 'max_new_tokens': 32768}
        ...
        generation_config, tokenizer, assistant_tokenizer, model_kwargs = self._prepare_generation(generation_config, use_model_defaults, assistant_model, **kwargs) #none none none {} -> cls {} none none

        # 2. Set generation parameters if not already defined
        synced_gpus, logits_processor, stopping_criteria, accepts_attention_mask, requires_attention_mask, kwargs_has_attention_mask = self._initialize_generation_parameters(synced_gpus=synced_gpus, logits_processor=logits_processor, stopping_criteria=stopping_criteria, **model_kwargs) #-> false [] [] true true true

        # 3. Define model inputs
        inputs_tensor, model_input_name, model_kwargs, batch_size, device= self._define_model_inputs(inputs, generation_config, kwargs_has_attention_mask, model_kwargs) #none cls true {} -> torch.Size([1, 18]) 'input_ids'  {'attention_mask'} 1 device(type='cuda', index=0)



        # 4. Define other model kwargs
        model_kwargs = self._define_other_model_kwargs( #->{'attention_mask'}
            generation_config, #class
            inputs_tensor, #torch.Size([1, 18])
            model_input_name, #'input_ids'
        ...
        input_ids, model_kwargs = self._prepare_input_ids_for_generation( #->torch.Size([1, 18]) {'attention_mask'}
            generation_config=generation_config, #class
            inputs_tensor=inputs_tensor, #torch.Size([1, 590, 768])
            model_input_name=model_input_name, #'inputs_embeds'

            model_kwargs=model_kwargs, #{inputs_embeds, attention_mask, encoder_outputs}
        ...
        input_ids_length = input_ids.shape[1] #->18
        ...
        model_kwargs["use_cache"] = generation_config.use_cache #model_kwargs添加了很多键值对
        ...
        prepared_logits_processor = self._get_logits_processor( #->[class]
            generation_config=generation_config, #class
            input_ids_seq_length=input_ids_length, #18
            encoder_input_ids=inputs_tensor, #torch.Size([1, 18])
            prefix_allowed_tokens_fn=prefix_allowed_tokens_fn, #None
            logits_processor=logits_processor, #[]
            device=inputs_tensor.device, #device(type='cuda', index=0)
            model_kwargs=model_kwargs,
            negative_prompt_ids=negative_prompt_ids,  #None
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result = self._handle_greedy_sample(
            ...
            result = self._handle_group_beam_search( #走这里
                generation_config,
                input_ids,
                inputs_tensor,
                prepared_logits_processor,
            ...
            result = self._handle_constrained_beam_search(
                input_ids=input_ids,
                inputs_tensor=inputs_tensor,
                model_kwargs=model_kwargs,
                generation_config=generation_config,
                batch_size=batch_size,
                prepared_logits_processor=prepared_logits_processor,
                prepared_stopping_criteria=prepared_stopping_criteria,
                synced_gpus=synced_gpus,
            )
    ...
    def _initialize_generation_state(self, generation_config, input_ids, model_kwargs, batch_size, cur_len):
        
        unfinished_sequences = torch.ones(batch_size, dtype=torch.long, device=input_ids.device) #torch.Size([1])
        this_peer_finished = False
        model_kwargs = self._get_initial_cache_position(cur_len, input_ids.device, model_kwargs) #->torch.Size([18])
        ...
        return model_forward, model_kwargs, unfinished_sequences, this_peer_finished, is_prefill



    def _run_token_generation_loop(
        self,
        input_ids,
        this_peer_finished,
        model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if is_prefill:
                outputs = self(**model_inputs, return_dict=True) #->输出类
                is_prefill = False
            else:
            ...
            # pre-process distribution
            next_token_scores = logits_processor(input_ids, next_token_logits) #torch.Size([1, 18]) torch.Size([1, 151936])->torch.Size([1, 151936])
            ...
            if do_sample:
                ...
                next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1) #->torch.Size([1])
            else:
                next_tokens = torch.argmax(next_token_scores, dim=-1)

            # finished sentences should have their next token be a padding token
            if has_eos_stopping_criteria:
                next_tokens = next_tokens * unfinished_sequences + pad_token_id * (1 - unfinished_sequences) #->torch.Size([1])
            ...
            unfinished_sequences = unfinished_sequences & ~stopping_criteria(input_ids, scores) #torch.Size([1])
            this_peer_finished = unfinished_sequences.max() == 0
            cur_len += 1 #记录当前的长度
        ...
        return input_ids, scores, raw_logits, decoder_attentions, cross_attentions, decoder_hidden_states
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # keep track of which sequences are already finished
        batch_size, cur_len = input_ids.shape[:2] #1 18
        model_forward, model_kwargs, unfinished_sequences, this_peer_finished, is_prefill = self._initialize_generation_state(
            generation_config, input_ids, model_kwargs, batch_size, cur_len
        ) #torch.Size([1, 18]) 1 18 class {} false


        input_ids, scores, raw_logits, decoder_attentions, cross_attentions, decoder_hidden_states = self._run_token_generation_loop(
            input_ids=input_ids,
            model_kwargs=model_kwargs,
