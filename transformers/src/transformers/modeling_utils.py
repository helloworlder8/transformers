...
def _get_resolved_checkpoint_files(
    ...
    """Get all the checkpoint filenames based on `model_name_or_path`, and optional metadata if the
    checkpoints are sharded.
    This function will download the data if necessary.
    """
    is_sharded = False

    if model_name_or_path is not None and gguf_file is None:
        model_name_or_path = str(model_name_or_path)
        is_local = os.path.isdir(model_name_or_path)
        if is_local:
            if transformers_explicit_filename is not None:
                # If the filename is explicitly defined, load this by default.
                archive_file = os.path.join(model_name_or_path, subfolder, transformers_explicit_filename)
                is_sharded = transformers_explicit_filename.endswith(".safetensors.index.json")
            elif from_tf and os.path.isfile(
                os.path.join(model_name_or_path, subfolder, TF_WEIGHTS_NAME + ".index")
            ):
                # Load from a TF 1.0 checkpoint in priority if from_tf
                archive_file = os.path.join(model_name_or_path, subfolder, TF_WEIGHTS_NAME + ".index")
            elif from_tf and os.path.isfile(os.path.join(model_name_or_path, subfolder, TF2_WEIGHTS_NAME)):
                # Load from a TF 2.0 checkpoint in priority if from_tf
                archive_file = os.path.join(model_name_or_path, subfolder, TF2_WEIGHTS_NAME)
            elif from_flax and os.path.isfile(
                os.path.join(model_name_or_path, subfolder, FLAX_WEIGHTS_NAME)
            ):
                # Load from a Flax checkpoint in priority if from_flax
                archive_file = os.path.join(model_name_or_path, subfolder, FLAX_WEIGHTS_NAME)
            elif use_safetensors is not False and os.path.isfile(
                os.path.join(model_name_or_path, subfolder, _add_variant(SAFE_WEIGHTS_NAME, variant))
            ):
                # Load from a safetensors checkpoint
                archive_file = os.path.join(
                    model_name_or_path, subfolder, _add_variant(SAFE_WEIGHTS_NAME, variant)
                )
            elif use_safetensors is not False and os.path.isfile(
                os.path.join(model_name_or_path, subfolder, _add_variant(SAFE_WEIGHTS_INDEX_NAME, variant))
            ):
                # Load from a sharded safetensors checkpoint
                archive_file = os.path.join(
                    model_name_or_path, subfolder, _add_variant(SAFE_WEIGHTS_INDEX_NAME, variant)
                )
                is_sharded = True
            elif not use_safetensors and os.path.isfile(
                os.path.join(model_name_or_path, subfolder, _add_variant(WEIGHTS_NAME, variant))
            ):
                # Load from a PyTorch checkpoint
                archive_file = os.path.join(
                    model_name_or_path, subfolder, _add_variant(WEIGHTS_NAME, variant)
                )
            elif not use_safetensors and os.path.isfile(
                os.path.join(model_name_or_path, subfolder, _add_variant(WEIGHTS_INDEX_NAME, variant))
            ):
                # Load from a sharded PyTorch checkpoint
                archive_file = os.path.join(
                    model_name_or_path, subfolder, _add_variant(WEIGHTS_INDEX_NAME, variant)
                )
                is_sharded = True
            # At this stage we don't have a weight file so we will raise an error.
            elif not use_safetensors and (
                os.path.isfile(os.path.join(model_name_or_path, subfolder, TF_WEIGHTS_NAME + ".index"))
                or os.path.isfile(os.path.join(model_name_or_path, subfolder, TF2_WEIGHTS_NAME))
            ):
                raise EnvironmentError(
                    f"Error no file named {_add_variant(WEIGHTS_NAME, variant)} found in directory"
                    f" {model_name_or_path} but there is a file for TensorFlow weights. Use"
                ...
                raise EnvironmentError(
                    f"Error no file named {_add_variant(WEIGHTS_NAME, variant)} found in directory"
                    f" {model_name_or_path} but there is a file for Flax weights. Use `from_flax=True`"
        ...
        elif os.path.isfile(os.path.join(subfolder, model_name_or_path)):
            archive_file = model_name_or_path
            is_local = True
        elif os.path.isfile(os.path.join(subfolder, model_name_or_path + ".index")):
            if not from_tf:
                raise ValueError(
                    f"We found a TensorFlow checkpoint at {model_name_or_path + '.index'}, please set "
                    "from_tf to True to load from this checkpoint."
                )
            archive_file = os.path.join(subfolder, model_name_or_path + ".index")
            is_local = True
        elif is_remote_url(model_name_or_path):
            filename = model_name_or_path
            resolved_archive_file = download_url(model_name_or_path)
        else:
            ...
            try:
                ...
                resolved_archive_file = cached_file(model_name_or_path, filename, **cached_file_kwargs)
                    ...
                    if resolved_archive_file is not None:
                        ...
                        if revision == "main":
                            resolved_archive_file, revision, is_sharded = auto_conversion(
                                model_name_or_path, **cached_file_kwargs
                            )
                        cached_file_kwargs["revision"] = revision
                        if resolved_archive_file is None:
                            raise EnvironmentError(
                                f"{model_name_or_path} does not appear to have a file named"
                        ...
                        filename = _add_variant(WEIGHTS_NAME, variant)
                        resolved_archive_file = cached_file(
                            model_name_or_path, filename, **cached_file_kwargs
                        )
                ...
                if not local_files_only and not is_offline_mode():
                    if resolved_archive_file is not None:
                        if filename in [WEIGHTS_NAME, WEIGHTS_INDEX_NAME]:
                            ...
                            if not has_file(model_name_or_path, safe_weights_name, **has_file_kwargs):
                                Thread(
                        ...
                        if has_file(model_name_or_path, TF2_WEIGHTS_NAME, **has_file_kwargs):
                            raise EnvironmentError(
                                f"{model_name_or_path} does not appear to have a file named"
                                f" {_add_variant(WEIGHTS_NAME, variant)} but there is a file for TensorFlow weights."
                                " Use `from_tf=True` to load this model from those weights."
                            )
                        elif has_file(model_name_or_path, FLAX_WEIGHTS_NAME, **has_file_kwargs):
                            raise EnvironmentError(
                                f"{model_name_or_path} does not appear to have a file named"
                            ...
                            model_name_or_path, WEIGHTS_NAME, **has_file_kwargs
                        ):
                            raise EnvironmentError(
                                f"{model_name_or_path} does not appear to have a file named"
                ...
                raise EnvironmentError(
                    f"Can't load the model for '{model_name_or_path}'. If you were trying to load it"
        ...
        # Load from URL or cache if already cached
            ...
            resolved_archive_file = cached_file(model_name_or_path, gguf_file, **cached_file_kwargs)
    ...
    if is_sharded:
        ...
        checkpoint_files = [resolved_archive_file] if model_name_or_path is not None else None
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
        ...
        if is_peft_available():
            ...
            if _adapter_model_path is not None and os.path.isfile(_adapter_model_path):
                with open(_adapter_model_path, "r", encoding="utf-8") as f:
                    _adapter_model_path = model_name_or_path
                    model_name_or_path = json.load(f)["base_model_name_or_path"]
        else:
            _adapter_model_path = None
            
        return _adapter_model_path, model_name_or_path
    ...
    @staticmethod
    def _configure_device_map(device_map):
    ...
    @staticmethod
    def _handle_quantization_config(load_in_4bit, load_in_8bit, quantization_config=None, **kwargs):
        ...
        return quantization_config, kwargs
    
                
                
    @classmethod
    def _load_config_and_kwargs(
        cls,
        config,
        model_name_or_path,
        cache_dir=None,
        ...
        **kwargs,
        ...
        """
        加载模型的配置（config）和其余未使用的模型参数（model_kwargs）。
        支持 config 为 PretrainedConfig 实例或路径/名称。
        """
    ...
    def _setup_quantizer(
        ...
        """
        设置量化器（hf_quantizer），更新 config、torch_dtype 和 device_map，并可选记录量化方法到 user_agent。

        参数:
            config: 模型配置对象
            quantization_config: 传入的量化配置
            torch_dtype: torch 数据类型
            from_tf: 是否来自 TensorFlow 权重
            from_flax: 是否来自 Flax 权重
            device_map: 设备映射配置
            weights_only: 是否只加载权重
            user_agent: 可选的 user_agent 字典，记录量化方法

        返回:
            config, hf_quantizer, torch_dtype, device_map
        """
        ...
        return config, hf_quantizer, torch_dtype, device_map, user_agent



    @staticmethod
    def _prepare_device_map_and_quantizer(
        model,
        config,
        device_map=None,
        max_memory=None,
        ...
        return device_map, keep_in_fp32_regex


    # model_name_or_path
    @classmethod  #关键词是类方法的话第一个参数传入的就是类
    def from_pretrained(
        ...
        **kwargs,
        ...
        """ 设备 """
        device_map = cls._configure_device_map(device_map)



        """ 量化配置 """
        quantization_config, kwargs = cls._handle_quantization_config(
            load_in_4bit, load_in_8bit, quantization_config, **kwargs
        )
        ...
        config, model_kwargs = cls._load_config_and_kwargs(
            config=config,
        ...
        config, hf_quantizer, torch_dtype, device_map, user_agent = cls._setup_quantizer(
            config=config,
        ...
        checkpoint_files, sharded_metadata = _get_resolved_checkpoint_files( #权重文件列表，是不是分片的
            model_name_or_path=model_name_or_path,
            subfolder=subfolder,
            variant=variant,
            gguf_file=gguf_file,
        ...
        is_from_file = model_name_or_path is not None or gguf_file is not None
        is_quantized = hf_quantizer is not None
        is_sharded = sharded_metadata is not None
        ...
        if (
            is_safetensors_available()
            and is_from_file #有文件
            and not is_sharded #权重文件是单一的
            and checkpoint_files[0].endswith(".safetensors")
        ):
            with safe_open(checkpoint_files[0], framework="pt") as f:
                metadata = f.metadata() #->{'format': 'pt'}
        ...
        """ 配置模型的一些属性 """
        device_map, keep_in_fp32_regex = cls._prepare_device_map_and_quantizer(
            model=model,
            config=config,
            device_map=device_map,
            max_memory=max_memory,
        ...
        if model.can_generate() and generation_config is not None:
        ...
        elif model.can_generate() and model_name_or_path is not None:
            repo_loading_kwargs = {
                "cache_dir": cache_dir,
                "force_download": force_download,
                "proxies": proxies,
            ...
            try:
                ...
                pass
            # Load custom generate function if `model_name_or_path` defines it (and override `generate`)
            if hasattr(model, "load_custom_generate"):
                try:
                    custom_generate = model.load_custom_generate(
                        model_name_or_path, trust_remote_code=trust_remote_code, **repo_loading_kwargs
                    )
