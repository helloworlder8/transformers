...
class EASamImageProcessor(BaseImageProcessor): #图像预处理的方法
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vision_pad_size = vision_pad_size
        ...
        size = size if size is not None else {"vision_longest_edge": 1024, "shortest_edge": 800, "longest_edge": 1333}
    ...
    def vision_resize(
        ...
        **kwargs,
        ...
        if "vision_longest_edge" not in size:
            raise ValueError(f"The `size` dictionary must contain the key `longest_edge`. Got {size.keys()}")
        input_size = get_image_size(image, channel_dim=input_data_format)
        output_height, output_width = self._get_preprocess_shape(input_size, size["vision_longest_edge"])
        return resize(
            image,
    ...
    def _handle_deprecated(self,kwargs,size):
        """
        Handles deprecated arguments and replaces them with updated ones.

        Args:
            kwargs (dict): Keyword arguments to be checked for deprecated arguments.

        Returns:
            tuple: Updated `kwargs` and the values of `do_pad` and `size` if they were provided.
        """
        import logging
        logger = logging.getLogger("deprecated_arguments")
    ...
    def _preprocess(
        ...
        if do_resize:
            image = self.vision_resize(image=image, size=size, resample=resample, input_data_format=input_data_format)
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        vision_model_input = self.vision_preprocess(
            images,
            segmentation_maps,
            do_resize,
            size,
        ...
        mm_model_input = self.mm_preprocess(
            images,
            annotations,
            return_segmentation_masks,
            masks_path,
        ...
        return vision_model_input, mm_model_input
