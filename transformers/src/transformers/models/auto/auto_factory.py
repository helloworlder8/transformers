...
FROM_PRETRAINED_TORCH_DOCSTRING = """
        ...
        passed as an argument or loaded from `model_name_or_path` if possible), or when it's missing, by
        falling back to using pattern matching on `model_name_or_path`:
...
class _BaseAutoModelClass:
    ...
    #             resolved_config_file = cached_file( #模型配置文件
    #                 model_name_or_path,
    #                 CONFIG_NAME,
    ...
    #         maybe_adapter_path = find_adapter_config_file( #none
    #             model_name_or_path, _commit_hash=commit_hash, **adapter_kwargs
    #         )
    ...
    #                 adapter_kwargs["_adapter_model_path"] = model_name_or_path
    ...
    #         config, kwargs = AutoConfig.from_pretrained( #获取模型配置内存空间
    #             model_name_or_path,
    ...
    #             class_ref, model_name_or_path, code_revision=code_revision, **hub_kwargs, **kwargs
    #         )
    ...
    @staticmethod
    def handle_hub_kwargs(kwargs):
        """Extract and process hub-related arguments."""
        hub_kwargs_names = [
            "cache_dir",
            "force_download",
            "local_files_only",
        ...
        if token is not None:
            hub_kwargs["token"] = token
        return hub_kwargs, token
    
    @staticmethod
    def resolve_commit_hash(config, model_name_or_path, commit_hash, hub_kwargs):
        """Resolve the commit hash of the model or configuration."""
        if commit_hash is None:
            if not isinstance(config, PretrainedConfig):
                resolved_config_file = cached_file( #取出配置文件
                    model_name_or_path,
                    CONFIG_NAME,
                    _raise_exceptions_for_gated_repo=False,
                    _raise_exceptions_for_missing_entries=False,
        ...
        if is_peft_available():
            ...
            if maybe_adapter_path is not None:
                with open(maybe_adapter_path, "r", encoding="utf-8") as f:
                    ...
                    adapter_kwargs["_adapter_model_path"] = model_name_or_path
                    model_name_or_path = adapter_config["base_model_name_or_path"]
        return model_name_or_path, adapter_kwargs

    @staticmethod
    def load_config(model_name_or_path, kwargs, hub_kwargs, commit_hash, trust_remote_code, code_revision):
        ...
        config, kwargs = AutoConfig.from_pretrained(
            model_name_or_path, #'ByteDance/Sa2VA-1B'
            return_unused_kwargs=True,
            trust_remote_code=trust_remote_code, #none
            code_revision=code_revision,#none
    ...
    @staticmethod
    def determine_model_class(config, cls, model_name_or_path, code_revision, hub_kwargs, kwargs, trust_remote_code):
        """Determine the appropriate model class based on the configuration."""
        has_remote_code = hasattr(config, "auto_map") and cls.__name__ in config.auto_map
        has_local_code = type(config) in cls._model_mapping.keys()
        trust_remote_code = resolve_trust_remote_code(
            trust_remote_code, model_name_or_path, has_local_code, has_remote_code
    ...
    @classmethod
    def from_pretrained(cls, model_name_or_path, *model_args, **kwargs):
        """Main entry point for loading a pretrained model."""
        kwargs["_from_auto"] = True
        config = kwargs.pop("config", None)
        trust_remote_code = kwargs.pop("trust_remote_code", None)
        hub_kwargs_names = [
        ...
        hub_kwargs = {name: kwargs.pop(name) for name in hub_kwargs_names if name in kwargs} #从kwargs中夺取hub_kwargs参数
        code_revision = kwargs.pop("code_revision", None)
        commit_hash = kwargs.pop("_commit_hash", None)
        adapter_kwargs = kwargs.pop("adapter_kwargs", None)#关键词减少计划
        token = hub_kwargs.pop("token", None)
        if token is not None:
            hub_kwargs["token"] = token
            
            
        commit_hash = cls.resolve_commit_hash(config, model_name_or_path, commit_hash, hub_kwargs) #none str none {}




        # Handle adapter kwargs
        model_name_or_path, adapter_kwargs = cls.handle_adapter_kwargs( #str none
        ...
        kwargs["adapter_kwargs"] = adapter_kwargs
        model_class = cls.determine_model_class( #公共的类到特定的类的转变
...
class _BaseAutoBackboneClass(_BaseAutoModelClass):
    ...
    @classmethod
    def _load_timm_backbone_from_pretrained(cls, model_name_or_path, *model_args, **kwargs):
    ...
    def from_pretrained(cls, model_name_or_path, *model_args, **kwargs):
        ...
        if use_timm_backbone:
            return cls._load_timm_backbone_from_pretrained(model_name_or_path, *model_args, **kwargs)
