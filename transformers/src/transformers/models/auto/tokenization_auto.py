...
def _get_tokenizer_class_from_type(tokenizer_type, use_fast): #none true
...
def tokenizer_class_str_2_tokenizer_class(tokenizer_class_str, use_fast):
...
def _get_tokenizer_config_and_auto_map(tokenizer_config_dict,model_name_or_path, config, gguf_file,trust_remote_code, kwargs):
    ...
    return tokenizer_auto_map, tokenizer_class_str, config #none str none




def tokenizer_auto_map_2_tokenizer_class(
...
class AutoTokenizer:
    ...
    def from_pretrained(cls, model_name_or_path, *inputs, **kwargs): #cls str () {}
        ...
        """ Get tokenizer class from tokenizer_type if provided """
        # Step 2: Get tokenizer class from tokenizer_type if provided
        tokenizer_type = kwargs.pop("tokenizer_type", None)
        use_fast = kwargs.pop("use_fast", True)
        tokenizer_class = _get_tokenizer_class_from_type(tokenizer_type, use_fast) #none true
        if tokenizer_type is not None:
        ...
        gguf_file = kwargs.get("gguf_file", None) #none
        ...
        tokenizer_class = tokenizer_auto_map_2_tokenizer_class(tokenizer_auto_map, config, use_fast, trust_remote_code, kwargs, model_name_or_path,tokenizer_class_str
        ) #远程是站在通义角度上在讲
        ...
        tokenizer_class = tokenizer_class_str_2_tokenizer_class(tokenizer_class_str, use_fast)
        if tokenizer_class is not None:
            return tokenizer_class.from_pretrained(model_name_or_path, *inputs, **kwargs)
