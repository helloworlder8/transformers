...
class CLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # 如果没有提供 inputs_embeds，则从 input_ids 获取嵌入
        if inputs_embeds is None:
            inputs_embeds = self.token_embedding(input_ids) #torch.Size([2, 7, 512])
            
        # 如果没有提供 position_ids，则使用默认的 position_ids
        if position_ids is None:
            position_ids = self.position_ids[:, :seq_len] #torch.Size([1, 7])
        # 获取位置嵌入
...
class CLIPSdpaAttention(CLIPAttention):
    ...
    def forward(
        ...
        query_states = self.q_proj(hidden_states) #->torch.Size([1, 50, 768])
        key_states = self.k_proj(hidden_states) #->torch.Size([1, 50, 768])
        value_states = self.v_proj(hidden_states) #->torch.Size([1, 50, 768])

        query_states = query_states.view(bsz, -1, self.num_heads, self.head_dim).transpose(1, 2) #->torch.Size([1, 12, 50, 64]) 批 头 序 维
...
class CLIPEncoder(nn.Module):
    ...
    def forward(
        ...
        return BaseModelOutput(
            last_hidden_state=hidden_states, #torch.Size([1, 50, 768])
...
class CLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        input_shape = input_ids.size() #torch.Size([2, 7])
        ...
        if attention_mask is not None and not self._use_flash_attention_2:
            ...
            attention_mask = _prepare_4d_attention_mask(attention_mask, hidden_states.dtype) #->torch.Size([2, 1, 7, 7]) 文bs 图bs 文长 文长

        encoder_outputs = self.encoder(
            hidden_states=hidden_states, #torch.Size([2, 7, 512])
            attention_mask=attention_mask, #torch.Size([2, 1, 7, 7])
            causal_attention_mask=causal_attention_mask, #torch.Size([2, 1, 7, 7])
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        )

        last_hidden_state = encoder_outputs[0] #-》torch.Size([2, 7, 512]) bs seq_len dim
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.embeddings(pixel_values, interpolate_pos_encoding=interpolate_pos_encoding) #打成patch后类别嵌入加上位置嵌入 torch.Size([1, 50, 768])
        hidden_states = self.pre_layrnorm(hidden_states) #->torch.Size([1, 50, 768])
        ...
        last_hidden_state = encoder_outputs[0] #torch.Size([1, 50, 768])
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        vision_outputs = self.vision_model(
            pixel_values=pixel_values, #torch.Size([1, 3, 224, 224])
        ...
        text_outputs = self.text_model(
            input_ids=input_ids, #[[len=7],[len=7]]
            attention_mask=attention_mask,
            position_ids=position_ids,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
        ...
        vision_embeds = self.visual_projection(vision_embeds) #torch.Size([1, 768])->torch.Size([1, 512])

        text_embeds = text_outputs[1]
        text_embeds = self.text_projection(text_embeds) #torch.Size([2, 512])->torch.Size([2, 512])

        # normalized features
        vision_embeds = vision_embeds / _get_vector_norm(vision_embeds)
        text_embeds = text_embeds / _get_vector_norm(text_embeds)

        # cosine similarity as logits
        logit_scale = self.logit_scale.exp()
        logits_per_text = torch.matmul(text_embeds, vision_embeds.t().to(text_embeds.device)) * logit_scale.to(text_embeds.device)
        logits_per_image = logits_per_text.t() #torch.Size([1, 2])
...
class CLIPTextModelWithProjection(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        return CLIPTextModelOutput(
            ...
            all_hidden_states=text_outputs.all_hidden_states,
            all_attentions=text_outputs.all_attentions,
        )
...
class CLIPVisionModelWithProjection(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        vision_embeds = self.visual_projection(pooled_output)
        ...
        return CLIPVisionModelOutput(
            ...
            all_hidden_states=vision_outputs.all_hidden_states,
...
class CLIPForImageClassification(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        image_features = torch.mean(image_features[:, 1:, :], dim=1)
        # apply classifier
...
__all__ = [
    "CLIPPreTrainedModel", #基础模型
    
    "CLIPModel", #论文中对比学习的模型

    "CLIPTextModel", #文本模型
    "CLIPTextModelWithProjection", #文本模型加投射
    
    "CLIPVisionModel", #视觉模型
    "CLIPVisionModelWithProjection", #视觉模型加投射
    
    "CLIPForImageClassification", #应用（图像分类）
]
