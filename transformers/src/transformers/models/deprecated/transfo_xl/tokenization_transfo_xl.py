...
class TransfoXLCorpus:
    ...
    def from_pretrained(cls, model_name_or_path, cache_dir=None, *inputs, **kwargs):
        ...
        """
        vocab = TransfoXLTokenizer.from_pretrained(model_name_or_path, *inputs, **kwargs)
        is_local = os.path.isdir(model_name_or_path)
        # redirect to the cache, if necessary
        try:
            resolved_corpus_file = cached_file(model_name_or_path, CORPUS_NAME, cache_dir=cache_dir)
        except EnvironmentError:
            logger.error(
                f"Corpus '{model_name_or_path}' was not found in corpus list"
                f" ({', '.join(PRETRAINED_CORPUS_ARCHIVE_MAP.keys())}. We assumed '{model_name_or_path}'"
