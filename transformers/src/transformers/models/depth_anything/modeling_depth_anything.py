...
class DepthAnythingReassembleStage(nn.Module):
    ...
    def forward(self, hidden_states: List[torch.Tensor], patch_height=None, patch_width=None) -> List[torch.Tensor]:
        ...
        for i, hidden_state_i in enumerate(hidden_states):
            # reshape to (batch_size, num_channels, height, width)
            hidden_state_i = hidden_state_i[:, 1:]
            batch_size, _, num_channels = hidden_state_i.shape
            hidden_state_i = hidden_state_i.reshape(batch_size, patch_height, patch_width, num_channels)
            hidden_state_i = hidden_state_i.permute(0, 3, 1, 2).contiguous() #torch.Size([1, 384, 37, 49])
            hidden_state_i = self.layers[i](hidden_state_i) #torch.Size([1, 48, 148, 196]) 卷积加上采样
            out.append(hidden_state_i)

        return out #torch.Size([1, 48, 148, 196]) torch.Size([1, 96, 74, 98]) torch.Size([1, 192, 37, 49]) torch.Size([1, 384, 19, 25])
...
class DepthAnythingNeck(nn.Module):
    ...
    * DepthAnythingFeatureFusionStage.
    ...
    def forward(self, hidden_states: List[torch.Tensor], patch_height=None, patch_width=None) -> List[torch.Tensor]:
        ...
        features = [self.convs[i](feature) for i, feature in enumerate(hidden_states)] #再加一层卷积

        # fusion blocks
        output = self.fusion_stage(features) # torch.Size([1, 64, 296, 392]) torch.Size([1, 64, 148, 196]) torch.Size([1, 64, 74, 98]) torch.Size([1, 64, 37, 49])
...
class DepthAnythingDepthEstimationHead(nn.Module):
    ...
    def forward(self, hidden_states: List[torch.Tensor], patch_height, patch_width) -> torch.Tensor:
        ...
        predicted_depth = self.conv1(hidden_states) #->torch.Size([1, 32, 296, 392])
        predicted_depth = nn.functional.interpolate( #->torch.Size([1, 32, 518, 686])
            predicted_depth,
        ...
        predicted_depth = predicted_depth.squeeze(dim=1)  # shape (batch_size, height, width) torch.Size([1, 518, 686])
...
class DepthAnythingForDepthEstimation(DepthAnythingPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.backbone.forward_with_filtered_kwargs(
            pixel_values, output_attentions=output_attentions, output_hidden_states=output_hidden_states
        )
        hidden_states = outputs.feature_maps

        _, _, height, width = pixel_values.shape
        patch_size = self.config.patch_size
        patch_height = height // patch_size #37
        patch_width = width // patch_size #49
