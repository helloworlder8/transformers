...
class DetrModel(DetrPreTrainedModel):
    ...
    def forward(
        ...
        return DetrModelOutput(
            ...
            enc_all_attentions=encoder_outputs.all_attentions,
...
class DetrForObjectDetection(DetrPreTrainedModel): #detr目标检测模型理解为项目或者工作流
    def __init__(self, config: DetrConfig):
        super().__init__(config)

        # DETR encoder-decoder model
        self.model = DetrModel(config) #整个模型
    ...
    def forward(
        ...
        outputs = self.model( #最重要前向传播
            pixel_values, #torch.Size([1, 3, 800, 1066])
            pixel_mask=pixel_mask, #torch.Size([1, 800, 1066])
