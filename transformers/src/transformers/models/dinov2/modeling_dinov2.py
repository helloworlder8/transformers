...
class Dinov2Embeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        new_height = height // self.patch_size #37
        new_width = width // self.patch_size #49

        sqrt_num_positions = torch_int(num_positions**0.5) #37
    ...
    def forward(self, pixel_values: torch.Tensor, bool_masked_pos: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        embeddings = self.patch_embeddings(pixel_values.to(dtype=target_dtype)) #torch.Size([1, 1813, 384]) bs num_patch D
...
class Dinov2Encoder(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, #torch.Size([1, 1814, 384])
        head_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,
        output_hidden_states: bool = False, #true
        ...
        for i, layer_i in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_i(hidden_states, layer_head_mask, output_attentions)
...
class Dinov2Backbone(Dinov2PreTrainedModel, BackboneMixin):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(pixel_values) #torch.Size([1, 3, 518, 686]) ->torch.Size([1, 1814, 384])

        outputs = self.encoder(
            embedding_output, output_hidden_states=True, output_attentions=output_attentions, return_dict=return_dict
        )

        all_hidden_states = outputs.all_hidden_states if return_dict else outputs[1]

        feature_maps = ()
        for stage, hidden_state_i in zip(self.stage_names, all_hidden_states):
            if stage in self.out_features:
                if self.config.apply_layernorm:
                    hidden_state_i = self.layernorm(hidden_state_i)
                if self.config.reshape_hidden_states:
                    ...
                    hidden_state_i = hidden_state_i.reshape(batch_size, height // patch_size, width // patch_size, -1)
                    hidden_state_i = hidden_state_i.permute(0, 3, 1, 2).contiguous()
                feature_maps += (hidden_state_i,)
        ...
        return BackboneOutput(
            ...
            all_attentions=outputs.all_attentions if output_attentions else None,
        )
