...
class DaViT(nn.Module):
    ...
    def forward_features_unpool(self, x):
        ...
        for conv, block in zip(self.convs, self.blocks):
            x, input_size = conv(x, input_size) #torch.Size([1, 36864, 128]) (192, 192)
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        if self._use_flash_attention_2:
            ...
            # the manual implementation that requires a 4D causal mask in all cases.
            decoder_attention_mask = _prepare_4d_causal_attention_mask_for_sdpa(
            ...
            attention_mask = _prepare_4d_causal_attention_mask(
                attention_mask, input_shape, decoder_inputs_embeds, past_key_values_length
            )
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        return Seq2SeqModelOutput(
            ...
            encoder_hidden_states=encoder_outputs.all_hidden_states,
            encoder_attentions=encoder_outputs.all_attentions,
        )
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def _encode_image(self, pixel_values): #torch.Size([1, 3, 768, 768])
        ...
        if self.image_pos_embed is not None:
            ...
            h, w = int(num_tokens ** 0.5), int(num_tokens ** 0.5)
            assert h * w == num_tokens, 'only support square feature maps for now'
            x = x.view(batch_size * T, h, w, x.shape[-1]) #torch.Size([1, 24, 24, 1024])
        ...
        spatial_avg_pool_x = x.view(batch_size, T, -1, x.shape[-1]).mean(dim=2)
        x_feat_dict['spatial_avg_pool'] = spatial_avg_pool_x #torch.Size([1, 1, 1024])
        ...
        **kwargs
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                inputs_embeds, attention_mask = self._merge_input_ids_with_image_features(image_features, inputs_embeds) #torch.Size([1, 590, 768]) torch.Size([1, 590])
