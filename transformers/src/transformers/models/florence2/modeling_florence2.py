...
class LearnedAbsolutePositionEmbedding2D(nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        x_emb = self.column_embeddings(width_values) #->torch.Size([24, 512])
        y_emb = self.row_embeddings(height_values) #->torch.Size([24, 512])
        # (height, width, embedding_dim * 2)
        pos = torch.cat([x_emb.unsqueeze(0).repeat(height, 1, 1), y_emb.unsqueeze(1).repeat(1, width, 1)], dim=-1)
        # (embedding_dim * 2, height, width)
        pos = pos.permute(2, 0, 1)
        pos = pos.unsqueeze(0) #->torch.Size([1, 1024, 24, 24])
...
class PositionalEmbeddingCosine1D(nn.Module):
    ...
    def forward(self, seq_embeds: torch.Tensor) -> torch.Tensor: #torch.Size([1, 1, 1024])
        """
        Args:
...
class ConvEmbed(nn.Module):
    ...
    def forward(self, x, size): #torch.Size([1, 3, 768, 768]) (768, 768)
        H, W = size # 768 768
        if len(x.size()) == 3:
        ...
        x = self.proj(x) #torch.Size([1, 128, 192, 192]) 核7步长为4
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, size):
        ...
        x = window_partition(x, self.window_size) #torch.Size([1, 192, 192, 128]) 12
...
class SpatialBlock(nn.Module):
    ...
    def forward(self, x, size): #torch.Size([1, 36864, 128]) (192, 192)
        if self.conv1:
            x, size = self.conv1(x, size) #torch.Size([1, 36864, 128]) (192, 192)
...
class DaViT(nn.Module):
    ...
    def forward_features_unpool(self, x): #torch.Size([1, 3, 768, 768])
        """
        ...
        for conv, block in zip(self.convs, self.blocks):
            x, input_size = conv(x, input_size) #->torch.Size([1, 36864, 128]) (192, 192)  torch.Size([1, 9216, 256]) (96, 96)
...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    """
    shifted_input_ids = input_ids.new_zeros(input_ids.shape) #->torch.Size([2, 7])
...
class Florence2LearnedPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids: torch.Tensor, past_key_values_length: int = 0):
        ...
        bsz, seq_len = input_ids.shape[:2] # 3 1
        positions = torch.arange(
            past_key_values_length, past_key_values_length + seq_len, dtype=torch.long, device=self.weight.device
        ).expand(bsz, -1) #torch.Size([3, 1])
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        output_attentions, output_hidden_states, return_dict = self._handle_params(output_attentions, output_hidden_states, return_dict) #false false true

        # retrieve input_ids and inputs_embeds
        if decoder_input_ids is not None and decoder_inputs_embeds is not None:
            raise ValueError("You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time")
        elif decoder_input_ids is not None:
            input = decoder_input_ids #torch.Size([3, 1])
            input_shape = input.shape #torch.Size([3, 1])
            decoder_input_ids = decoder_input_ids.view(-1, input_shape[-1])
        elif decoder_inputs_embeds is not None:
            input_shape = decoder_inputs_embeds.size()[:-1]
            input = decoder_inputs_embeds[:, :, -1]
        else:
        ...
        if decoder_inputs_embeds is None:
            decoder_inputs_embeds = self.embed_tokens(input) #torch.Size([3, 1, 768])

        if self._use_flash_attention_2:
            # 2d mask is passed through the layers
            decoder_attention_mask = decoder_attention_mask if (decoder_attention_mask is not None and 0 in decoder_attention_mask) else None
        elif self._use_sdpa and not output_attentions and cross_attn_head_mask is None:
            # output_attentions=True & cross_attn_head_mask can not be supported when using SDPA, and we fall back on
            # the manual implementation that requires a 4D causal mask in all cases.
            decoder_attention_mask = _prepare_4d_causal_attention_mask_for_sdpa( #->none
                decoder_attention_mask,
                input_shape, #torch.Size([3, 1])
                decoder_inputs_embeds, #torch.Size([3, 1, 768])
                past_key_values_length, #0
            )
        else:
            # 4d mask is passed through the layers
            decoder_attention_mask = _prepare_4d_causal_attention_mask(
                decoder_attention_mask, input_shape, decoder_inputs_embeds, past_key_values_length
            )
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            if self._use_flash_attention_2:
                ...
                # the manual implementation that requires a 4D causal mask in all cases.
                # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
                encoder_attention_mask = _prepare_4d_attention_mask_for_sdpa( #->none
                    encoder_attention_mask,
                    decoder_inputs_embeds.dtype,
                    tgt_len=input_shape[-1],
                )
            else:
                # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
                encoder_attention_mask = _prepare_4d_attention_mask(
                    encoder_attention_mask, decoder_inputs_embeds.dtype, tgt_len=input_shape[-1]
                )

        # embed positions
        positions = self.embed_positions(input, past_key_values_length) #torch.Size([3, 1, 768])
        ...
        for attn_mask, mask_name in zip([decoder_head_mask, cross_attn_head_mask], ["head_mask", "cross_attn_head_mask"]):
            if attn_mask is not None:
                if attn_mask.size()[0] != (len(self.layers)):
                    raise ValueError(
                        f"The `{mask_name}` should be specified for {len(self.layers)} layers, but it is for"
                        f" {decoder_head_mask.size()[0]}."
                    )
        ...
        for idx, layer_i in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(
                    ...
                    encoder_attention_mask,
                    decoder_head_mask[idx] if decoder_head_mask is not None else None,
                ...
                layer_outputs = layer_i(
                    ...
                    layer_head_mask=(decoder_head_mask[idx] if decoder_head_mask is not None else None),
...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None, #torch.Size([3, 590])
        decoder_input_ids: Optional[torch.LongTensor] = None, #torch.Size([3, 1])
        decoder_attention_mask: Optional[torch.LongTensor] = None,
        head_mask: Optional[torch.Tensor] = None,
        decoder_head_mask: Optional[torch.Tensor] = None,
        cross_attn_head_mask: Optional[torch.Tensor] = None,
        encoder_outputs: Optional[List[torch.FloatTensor]] = None, #dict
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        decoder_inputs_embeds: Optional[torch.FloatTensor] = None,
        use_cache: Optional[bool] = None,
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                inputs_embeds=inputs_embeds, #torch.Size([2, 595, 768])
                attention_mask=attention_mask, #torch.Size([2, 595])
        ...
        decoder_outputs = self.decoder(
            decoder_input_ids=decoder_input_ids, #torch.Size([3, 1])
            decoder_attention_mask=decoder_attention_mask, #none
            encoder_hidden_states=encoder_outputs[0], #torch.Size([3, 590, 768])
            encoder_attention_mask=attention_mask, #torch.Size([3, 590])
            ...
            decoder_inputs_embeds=decoder_inputs_embeds, #none
        ...
        return Seq2SeqModelOutput(
            ...
            decoder_all_hidden_states=decoder_outputs.all_hidden_states,
            decoder_all_attentions=decoder_outputs.all_attentions,
            decoder_all_cross_attentions=decoder_outputs.all_cross_attentions,
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        outputs = self.model(
            input_ids, #None
            attention_mask=attention_mask, #torch.Size([3, 590])
            decoder_input_ids=decoder_input_ids, #torch.Size([3, 1])
            encoder_outputs=encoder_outputs, #dict
            decoder_attention_mask=decoder_attention_mask, #none
            head_mask=head_mask,#none
            decoder_head_mask=decoder_head_mask,#none
            cross_attn_head_mask=cross_attn_head_mask,#none
        ...
        return Seq2SeqLMOutput(
            ...
            decoder_all_hidden_states=outputs.decoder_all_hidden_states,
            decoder_all_attentions=outputs.decoder_all_attentions,
            decoder_all_cross_attentions=outputs.decoder_all_cross_attentions,
            
            encoder_last_hidden_state=outputs.encoder_last_hidden_state,
            encoder_all_hidden_states=outputs.encoder_all_hidden_states,
            encoder_all_attentions=outputs.encoder_all_attentions,
        )
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def _encode_image(self, pixel_values): #torch.Size([1, 3, 768, 768])
        if len(pixel_values.shape) == 4:
            batch_size, C, H, W = pixel_values.shape
            T = 1
            x = self.vision_tower.forward_features_unpool(pixel_values) #torch.Size([1, 576, 1024])
        else:
            raise ValueError(f'invalid image shape {pixel_values.shape}')
        
        if self.image_pos_embed is not None:
            x = x.view(batch_size * T, -1, x.shape[-1]) #torch.Size([1, 576, 1024])
            num_tokens = x.shape[-2] #->576
            h, w = int(num_tokens ** 0.5), int(num_tokens ** 0.5)
            assert h * w == num_tokens, 'only support square feature maps for now'
            x = x.view(batch_size * T, h, w, x.shape[-1]) #torch.Size([1, 24, 24, 1024]) #还原到类似yolo的特征图
            pos_embed = self.image_pos_embed(x) #torch.Size([1, 24, 24, 1024])
            x = x + pos_embed
            x = x.view(batch_size, T * h*w, x.shape[-1]) #->torch.Size([1, 576, 1024]) bs hw(token) dim

        if self.visual_temporal_embed is not None:
            visual_temporal_embed = self.visual_temporal_embed(x.view(batch_size, T, -1, x.shape[-1])[:, :, 0])
            x = x.view(batch_size, T, -1, x.shape[-1]) + visual_temporal_embed.view(1, T, 1, x.shape[-1]) #->torch.Size([1, 1, 576, 1024])

        x_feat_dict = {}

        spatial_avg_pool_x = x.view(batch_size, T, -1, x.shape[-1]).mean(dim=2)
        x_feat_dict['spatial_avg_pool'] = spatial_avg_pool_x #torch.Size([1, 1, 1024]) 每一个通道像素取平均值

        temporal_avg_pool_x = x.view(batch_size, T, -1, x.shape[-1]).mean(dim=1)
        x_feat_dict['temporal_avg_pool'] = temporal_avg_pool_x #torch.Size([1, 576, 1024])

        x = x.view(batch_size, T, -1, x.shape[-1])[:, -1] #->torch.Size([1, 1, 576, 1024]) -> torch.Size([1, 576, 1024])
        x_feat_dict['last_frame'] = x
        ...
        x = torch.cat(new_x, dim=1) #torch.Size([1, 577, 1024])

        x = x @ self.image_projection #torch.Size([1024, 768]) ->torch.Size([1, 577, 768])

        x = self.image_proj_norm(x)

        return x  #torch.Size([1, 577, 768]) bas token dim

    def _merge_input_ids_with_image_features(
        self, image_features, inputs_embeds #torch.Size([1, 577, 768]) torch.Size([1, 13, 768])
    ):
        batch_size, image_token_length = image_features.size()[:-1] # 1 577
        device = image_features.device #device(type='cuda', index=0)
        image_attention_mask = torch.ones(batch_size, image_token_length, device=device) #->torch.Size([1, 577])
        ...
        task_prefix_attention_mask = torch.ones(batch_size, task_prefix_embeds.size(1), device=device) #torch.Size([1, 13])
        ...
        return inputs_embeds, attention_mask #torch.Size([1, 590, 768]) torch.Size([1, 590])
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if input_ids is not None:
                inputs_embeds = self.get_input_embeddings()(input_ids) #torch.Size([2, 18]) -> torch.Size([2, 18, 768])
            # 2. Merge text and images
            if pixel_values is not None:
                # (batch_size, num_image_tokens, hidden_size)
                image_features = self._encode_image(pixel_values) #torch.Size([2, 3, 768, 768])-> torch.Size([2, 577, 768])
                inputs_embeds, attention_mask = self._merge_input_ids_with_image_features(image_features, inputs_embeds) #-》torch.Size([2, 595, 768]) torch.Size([2, 595])
        ...
        return Florence2Seq2SeqLMOutput(
            ...
            decoder_hidden_states=outputs.decoder_all_hidden_states,
            decoder_attentions=outputs.decoder_all_attentions,
    ...
    def generate(
        ...
        **kwargs
        ...
        if inputs_embeds is None:
            # 1. Extra the input embeddings
            if input_ids is not None:
                inputs_embeds = self.get_input_embeddings()(input_ids) #torch.Size([1, 13, 768]) 获取词嵌入
            # 2. Merge text and images
            if pixel_values is not None:
                image_features = self._encode_image(pixel_values) #torch.Size([1, 3, 768, 768]) ->torch.Size([1, 577, 768])
                inputs_embeds, attention_mask = self._merge_input_ids_with_image_features(image_features, inputs_embeds) # torch.Size([1, 577, 768]) torch.Size([1, 13, 768])
        
        return self.language_model.generate( #Florence2LanguageForConditionalGeneration
            input_ids=None,
            inputs_embeds=inputs_embeds, #torch.Size([1, 590, 768])
            **kwargs #{'max_new_tokens': 1024, 'do_sample': False, 'num_beams': 3}
        )
