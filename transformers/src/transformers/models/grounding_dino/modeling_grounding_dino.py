...
class GroundingDinoDecoderOutput(ModelOutput):
    ...
    """
    dec_query_embeds_states: torch.FloatTensor = None
    dec_query_embeds_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    dec_refine_coords_all_hidden_states: torch.FloatTensor = None
    dec_all_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class GroundingDinoEncoderOutput(ModelOutput):
    ...
    """
    enc_vision_features_states: torch.FloatTensor = None
    enc_text_features_states: torch.FloatTensor = None
    
    enc_vision_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    enc_text_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    
    enc_all_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class GroundingDinoModelOutput(ModelOutput):
    ...
    """
    dec_query_embeds_states: torch.FloatTensor = None
    dec_query_embeds_all_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    dec_refine_coords_all_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    dec_all_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None

    enc_vision_features_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    enc_text_features_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    enc_vision_all_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    enc_text_all_hidden_states: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
    enc_all_attentions: Optional[Tuple[Tuple[torch.FloatTensor]]] = None
...
class GroundingDinoObjectDetectionOutput(ModelOutput):
    ...
    dec_query_embeds_states: Optional[Tuple[torch.FloatTensor]] = None
    dec_query_embeds_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    dec_refine_coords_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    dec_all_attentions: Optional[Tuple[torch.FloatTensor]] = None
    
    enc_vision_features_states: Optional[Tuple[torch.FloatTensor]] = None
    enc_text_features_states: Optional[Tuple[torch.FloatTensor]] = None
    enc_vision_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    enc_text_all_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    enc_all_attentions: Optional[Tuple[torch.FloatTensor]] = None
...
class GroundingDinoConvModel(nn.Module):
    ...
    def forward(self, pixel_values, pixel_mask):
        ...
        feature_maps_masks = self.conv_encoder(pixel_values, pixel_mask)
        posi_embed = []
        for feature_map, mask in feature_maps_masks:
            # position encoding
            posi_embed.append(self.position_embedding(feature_map, mask).to(feature_map.dtype))

        return feature_maps_masks, posi_embed #特征层，掩码  位置编码
...
class GroundingDinoMultiscaleDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        vision_attention_masks: Optional[torch.Tensor] = None,
        ...
        if vision_attention_masks is not None:
            value = value.masked_fill(~vision_attention_masks[..., None], 0.0)
...
class GroundingDinoTextEnhancerLayer(nn.Module):
    ...
    def forward(
        self,
        text_features: torch.FloatTensor, #torch.Size([1, 9, 256])
        text_self_attention_mask: Optional[torch.BoolTensor] = None, #torch.Size([1, 9, 9])
        text_posi_embed: Optional[torch.FloatTensor] = None, #torch.Size([1, 9, 256])
        ...
        """ 执行自注意力 """
        attention_output, text_enhanced_attn = self.self_attn(
            queries=queries,
            keys=keys,
        ...
        return text_features, text_enhanced_attn
...
class GroundingDinoBiMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        vision_attention_masks: Optional[torch.BoolTensor] = None,
        ...
        text_value = self._reshape(self.values_text_proj(text_features), -1, batch_size) #->torch.Size([1, 4, 9, 256])

        # Reshape for multi-head attention
        proj_shape = (batch_size * self.num_heads, -1, self.head_dim) #(4, -1, 256)
        ...
        # Apply masks
        if vision_attention_masks is not None:
            vision_attention_masks = vision_attention_masks[:, None, None, :].repeat(1, self.num_heads, 1, 1).flatten(0, 1)
            text_fused_attn.masked_fill_(vision_attention_masks, float("-inf"))
        ...
        vision_enhance_features = torch.bmm(vision_attn_probs, text_value) #torch.Size([4, 17821, 256])
        text_enhance_features = torch.bmm(text_attn_probs, vision_value) #->torch.Size([4, 9, 256])


        # Reshape and combine heads
        vision_enhance_features = self._combine_heads(vision_enhance_features, batch_size, vision_seq_len) #->torch.Size([1, 17821, 1024])
        text_enhance_features = self._combine_heads(text_enhance_features, batch_size, text_seq_len) #->torch.Size([1, 9, 1024])

        # Final linear projections (FFN)
        vision_enhance_features = self.out_vision_proj(vision_enhance_features)
        text_enhance_features = self.out_text_proj(text_enhance_features)


        return (vision_enhance_features, vision_fused_attn), (text_enhance_features, text_fused_attn)
...
class GroundingDinoFusionLayer(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            vision_attention_masks (`torch.BoolTensor`, **optional**):
        ...
        (vision_enhance_features, vision_fused_attn), (text_enhance_features, text_fused_attn) = self.attn(
            vision_features,
            text_features,
            vision_attention_masks,
            text_attention_mask,
        )
        vision_features = vision_features + self.drop_path(self.vision_param * vision_enhance_features)
        text_features = text_features + self.drop_path(self.text_param * text_enhance_features)
...
class GroundingDinoDeformableLayer(nn.Module):
    ...
    def forward(
        ...
        vision_attention_masks: torch.Tensor,
        ...
        vision_features, vision_deformable_attn = self.self_attn(
            ...
            vision_attention_masks=vision_attention_masks,
...
class GroundingDinoEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        vision_attention_masks, #->torch.Size([1, 17821])
        ...
        """ 特征融合 """
        (vision_features, vision_fused_attn), (text_features, text_fused_attn) = self.fusion_layer( #->torch.Size([1, 17821, 256]) torch.Size([4, 17821, 9])   torch.Size([1, 9, 256]) torch.Size([4, 9, 17821])
            vision_features=vision_features, #torch.Size([1, 17821, 256]) 
            text_features=text_features, #torch.Size([1, 9, 256])
            vision_attention_masks=vision_attention_masks, #torch.Size([1, 17821])
...
class GroundingDinoDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        query_embeds, query2text_cross_attn = self.encoder_attn_text(
            queries=queries,
            keys=enc_text_features,
            values=enc_text_features,
        ...
        query_embeds, query2vision_cross_attn = self.encoder_attn(
            ...
            vision_features=enc_vision_features,
            vision_attention_masks=vision_attention_masks,
...
# enc_vision_features
    def forward(self, vision_hidden_state, text_hidden_state, text_attention_mask):
        # Compute similarity between vision and text hidden states
        output = vision_hidden_state @ text_hidden_state.transpose(-1, -2) #->torch.Size([1, 17821, 9])
        output = output.masked_fill(~text_attention_mask[:, None, :], float("-inf"))
        ...
        return padded_output #->torch.Size([1, 17821, 256])
...
GROUNDING_DINO_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_outputs (`tuple(tuple(torch.FloatTensor)`, *optional*):
            Tuple consists of (`enc_vision_features_states`, *optional*: `enc_text_features_states`, *optional*:
            `vision_hidden_states`, *optional*: `text_hidden_states`, *optional*: `attentions`)
            `enc_vision_features_states` of shape `(batch_size, vision_seq_len, hidden_size)`, *optional*) is a sequence
...
class GroundingDinoEncoder(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        anchor_points = self.generate_anchor_points(vision_map_shapes, valid_ratios, device=vision_features.device) #torch.Size([4, 2]) torch.Size([1, 4, 2]) ->torch.Size([1, 17821, 4, 2]) 特征图中每一个像素点生成4个anchor points


        enc_all_attentions = () if output_attentions else None
        ...
        for i, layers_i in enumerate(self.layers):
            """ 无需了解编码器中的隐藏层状态 """
            if output_hidden_states: #相当于从0层开始
        ...
        if output_hidden_states: #这么变成0到最后一层都有
            vision_hidden_states += (vision_features,)
            text_hidden_states += (text_features,)

        if output_attentions: #所有注意力集合
            enc_all_attentions = (vision_fused_attn, text_fused_attn, text_enhanced_attn, vision_deformable_attn)

        if not return_dict:
            enc_outputs = [vision_features, text_features, vision_hidden_states, text_hidden_states, enc_all_attentions]
            return tuple(v for v in enc_outputs if v is not None)
        return GroundingDinoEncoderOutput(
            enc_vision_features_states=vision_features, #torch.Size([1, 17821, 256]) 视觉特征
            enc_text_features_states=text_features, #torch.Size([1, 9, 256]) 文本特征

            enc_vision_all_hidden_states=vision_hidden_states, #none
            enc_text_all_hidden_states=text_hidden_states, #none
            
            enc_all_attentions=enc_all_attentions, #none 有的话是所有的注意力
        )
...
class GroundingDinoDecoder(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        topk_coords=None,
        #enc_vision_features
        enc_vision_features=None,
        vision_attention_masks=None ,
        enc_text_features=None,
        ...
        dec_all_attentions = () if output_attentions else None
        all_cross_attns_vision = () if (output_attentions and enc_vision_features is not None) else None
        all_cross_attns_text = () if (output_attentions and enc_text_features is not None) else None
        query_embeds_hidden = ()
        refine_coords_hidden = ()

        if text_attention_mask is not None:
            dtype = enc_text_features.dtype
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer( #重点
                    query_embeds=query_embeds, #torch.Size([1, 900, 256])
                    query_posi_embeds=query_posi_embeds, #torch.Size([1, 900, 256])
                    expand_topk_coords=expand_topk_coords, #torch.Size([1, 900, 4, 4])

                    enc_vision_features=enc_vision_features,
                    vision_attention_masks=vision_attention_masks,
                    enc_text_features=enc_text_features,
            ...
            if output_attentions:
                ...
                if enc_text_features is not None:
                    all_cross_attns_text += (layer_outputs[2],)

                if enc_vision_features is not None:
        ...
        if output_attentions:
            dec_all_attentions += (all_self_attns, all_cross_attns_text, all_cross_attns_vision)


        if return_dict:
            return GroundingDinoDecoderOutput(
                    dec_query_embeds_states=query_embeds,
                    dec_query_embeds_all_hidden_states=query_embeds_hidden, #torch.Size([1, 6, 900, 256])
                    dec_refine_coords_all_hidden_states=refine_coords_hidden, #torch.Size([1, 6, 900, 4])
                    dec_all_attentions=dec_all_attentions,
                )
        return tuple(
            v
            for v in [
...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    ...
    def encoder_query_proposals(self, enc_vision_features, vision_attention_masks, vision_map_shapes):
        batch_size = enc_vision_features.shape[0]
        proposals = []
        current_position = 0

        for level, (height, width) in enumerate(vision_map_shapes):
            # Reshape and process the attention mask for the current level
            lvl_vision_attention_mask = vision_attention_masks[:, current_position : current_position + height * width]
            ...
            grid_y, grid_x = meshgrid(
                torch.linspace(0, height - 1, height, dtype=torch.float32, device=enc_vision_features.device),
                torch.linspace(0, width - 1, width, dtype=torch.float32, device=enc_vision_features.device),
                indexing="ij",
            )
            ...
            lvl_width_height = torch.ones_like(lvl_grid) * 0.05 * (2.0 ** level)
            lvl_proposals = torch.cat((lvl_grid, lvl_width_height), dim=-1).view(batch_size, -1, 4)
            proposals.append(lvl_proposals)
        ...
        valid_proposals = valid_proposals.masked_fill(vision_attention_masks.unsqueeze(-1), float("inf"))
        valid_proposals = valid_proposals.masked_fill(~output_proposals_valid, float("inf"))

        # Assign each pixel as an object query
        valid_object_query = enc_vision_features
        valid_object_query = valid_object_query.masked_fill(vision_attention_masks.unsqueeze(-1), 0.0)
        ...
        return valid_object_query, valid_proposals #torch.Size([1, 17821, 256])  torch.Size([1, 17821, 4])
    ...
    def _prepare_text_inputs(
        ...
        text_self_attention_mask, position_ids = generate_text_self_attention_masks(input_ids) #torch.Size([1, 9]) 开始结束加两句话
        ...
        return (
            input_ids,  #torch.Size([1, 9])
            token_type_ids, #哪句话 torch.Size([1, 9])
            text_attention_mask, # 好像都值得关注 torch.Size([1, 9])
            
            text_self_attention_mask, #大矩阵 torch.Size([1, 9, 9])
            position_ids, #一句话的分隔 torch.Size([1, 9])
        )
    ...
    def _extract_text_features(
        ...
        return text_features #torch.Size([1, 9, 256])
    ...
    def _extract_vision_features(
        ...
        feature_maps_masks, vision_posi_embed = self.backbone(pixel_values, pixel_mask)

        # 命名规范 特征图将层级 输出讲隐藏层
        vision_features = []
        vision_attention_masks = []
        for level, (feature_map_i, mask_i) in enumerate(feature_maps_masks):
            vision_features.append(self.input_proj_vision[level](feature_map_i))
            vision_attention_masks.append(mask_i)
        ...
        if self.config.num_feature_levels > len(vision_features):
            for level in range(len(vision_features), self.config.num_feature_levels): #
                if level == len(vision_features):
                    feature_map_i = self.input_proj_vision[level](feature_maps_masks[-1][0])
                else:
                    feature_map_i = self.input_proj_vision[level](vision_features[-1])
                    
                mask_i = nn.functional.interpolate(
                    pixel_mask.unsqueeze(1).float(),
                    size=feature_map_i.shape[-2:],
                    mode="nearest"
                ).to(torch.bool).squeeze(1)
                
                vision_posi_embed_i = self.backbone.position_embedding(feature_map_i, mask_i).to(feature_map_i.dtype)

                vision_features.append(feature_map_i)
                vision_attention_masks.append(mask_i)
                vision_posi_embed.append(vision_posi_embed_i)
                


        return vision_features, vision_attention_masks, vision_posi_embed


    def _vision_features_handle(
        self,
        vision_features: List[torch.Tensor],
        vision_attention_masks: List[torch.Tensor],
        ...
        for level, (vision_features_i, vision_attention_mask_i, vision_posi_embed_i) in enumerate(zip(vision_features, vision_attention_masks, vision_posi_embed)):
        ...
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in vision_attention_masks], 1).float()
    ...
    # anchor机制 编码类别 坐标信息 query_embeds不做操作
    def _prepare_decoder_inputs(
        self,
        query_embeds, #torch.Size([900, 256])
        encoder_outputs: Union[GroundingDinoEncoderOutput, Tuple[torch.Tensor, ...]], #class
        vision_attention_masks : torch.Tensor, #torch.Size([1, 17821])
        vision_map_shapes: torch.Tensor, #torch.Size([4, 2])
        text_attention_mask : torch.Tensor,  #torch.Size([1, 9])
        batch_size: int, #1
        ...
        if self.config.two_stage:
            ...
            valid_object_query, valid_proposals = self.encoder_query_proposals(
                encoder_outputs[0], ~vision_attention_masks, vision_map_shapes
            ) #torch.Size([1, 17821, 256]) torch.Size([1, 17821]) torch.Size([4, 2])
            #->torch.Size([1, 17821, 256]) torch.Size([1, 17821, 4])
            ...
            ) #torch.Size([1, 17821, 256]) torch.Size([1, 9, 256]) torch.Size([1, 9])
            ...
            topk_coords = topk_coords_logits.sigmoid() #torch.Size([1, 900, 4])
            init_topk_coords = topk_coords #torch.Size([1, 900, 4])
    ...
    def forward(
        ...
        pixel_values: Tensor, #torch.Size([1, 3, 800, 1066])
        pixel_mask: Optional[Tensor] = None, #torch.Size([1, 800, 1066])
        
        input_ids: Optional[Tensor] = None, #torch.Size([1, 9])
        token_type_ids: Optional[Tensor] = None, #torch.Size([1, 9])
        text_attention_mask: Optional[Tensor] = None, #torch.Size([1, 9])
        ...
        (
            input_ids, #torch.Size([1, 9])
            token_type_ids, #哪句话 torch.Size([1, 9])
            text_attention_mask, # 哪一个词值得关注 torch.Size([1, 9])
        ...
        # Step 4: 提取视觉特征
        vision_features, vision_attention_masks, vision_posi_embed = self._extract_vision_features(pixel_values, pixel_mask) #torch.Size([1, 3, 800, 1066]) torch.Size([1, 800, 1066])  -> [torch.Size([1, 256, 100, 134]) torch.Size([1, 256, 50, 67]) torch.Size([1, 256, 25, 34]) torch.Size([1, 256, 13, 17])]     [torch.Size([1, 256, 100, 134]) torch.Size([1, 256, 50, 67]) torch.Size([1, 256, 25, 34]) torch.Size([1, 256, 13, 17])]    [torch.Size([1, 256, 100, 134]) torch.Size([1, 256, 50, 67]) torch.Size([1, 256, 25, 34]) torch.Size([1, 256, 13, 17])] 
        # Step 5: 视觉特征进行处理
        (
            batch_size, #1
            vision_features , #torch.Size([1, 17821, 256]) 特征
            vision_attention_masks , #torch.Size([1, 17821]) mask
            vision_posi_embed, #torch.Size([1, 17821, 256]) 嵌入
            
            vision_map_shapes, #torch.Size([4, 2])
            level_start_index, #torch.Size([4])
            valid_ratios, #torch.Size([1, 4, 2])
        ) = self._vision_features_handle(vision_features, vision_attention_masks, vision_posi_embed) #简答理解为展平后的结果
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                vision_map_shapes=vision_map_shapes, #torch.Size([4, 2]) #形
                level_start_index=level_start_index, #torch.Size([4]) #开
                valid_ratios=valid_ratios, #torch.Size([1, 4, 2]) #比
                ...
                text_self_attention_mask=~text_self_attention_mask, #torch.Size([1, 9, 9])
                text_position_ids=position_ids, #torch.Size([1, 9])

                return_dict=return_dict, #true
                output_attentions=output_attentions, #false
                output_hidden_states=output_hidden_states, #falses
            )
        elif return_dict and not isinstance(encoder_outputs, GroundingDinoEncoderOutput):
            encoder_outputs = GroundingDinoEncoderOutput(
                enc_vision_features_states=encoder_outputs[0],
                enc_text_features_states=encoder_outputs[1],
        ...
        if self.config.embedding_init_target or self.config.two_stage:
            query_embeds = self.query_position_embeddings.weight #获取查询嵌入 torch.Size([900, 256])
        # Step 7: Prepare decoder inputs
        (
            query_embeds,     #torch.Size([1, 900, 256])
            topk_coords,      #torch.Size([1, 900, 4])
            init_topk_coords, #torch.Size([1, 900, 4])
            enc_outputs_class, #torch.Size([1, 17821, 256])
            enc_outputs_coord, #torch.Size([1, 17821, 4])
        ) = self._prepare_decoder_inputs(query_embeds, encoder_outputs, vision_attention_masks , vision_map_shapes, text_attention_mask, batch_size) #  torch.Size([900, 256]) class  torch.Size([1, 17821]) torch.Size([4, 2]) torch.Size([1, 9])  1 
        # Step 8: Pass through decoder
        decoder_outputs = self.decoder(
            query_embeds=query_embeds, #torch.Size([1, 900, 256])
            topk_coords=topk_coords, #torch.Size([1, 900, 4])
            
            enc_vision_features=encoder_outputs[0], #torch.Size([1, 17821, 256])
            vision_attention_masks=vision_attention_masks , #torch.Size([1, 17821])
            enc_text_features=encoder_outputs[1], #torch.Size([1, 9, 256])
            text_attention_mask=~text_attention_mask, #torch.Size([1, 9])
        ...
        return GroundingDinoModelOutput(
            ...
            dec_query_embeds_states=decoder_outputs.dec_query_embeds_states, #torch.Size([1, 900, 256])
            dec_query_embeds_all_hidden_states=decoder_outputs.dec_query_embeds_all_hidden_states, #torch.Size([1, 6, 900, 256])
            dec_refine_coords_all_hidden_states=decoder_outputs.dec_refine_coords_all_hidden_states, #torch.Size([1, 6, 900, 4])
            dec_all_attentions=decoder_outputs.dec_all_attentions,


            enc_vision_features_states=encoder_outputs.enc_vision_features_states,
            enc_text_features_states=encoder_outputs.enc_text_features_states,
            enc_vision_all_hidden_states=encoder_outputs.enc_vision_all_hidden_states,
            enc_text_all_hidden_states=encoder_outputs.enc_text_all_hidden_states,
            enc_all_attentions=encoder_outputs.enc_all_attentions,
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel): #理解为整个项目
    ...
    # 输出是编码器和解码器的输出
    def _process_outputs(self, outputs, output_attentions, output_hidden_states, return_dict):
        if return_dict:
            enc_text_features_states = outputs.enc_text_features_states #torch.Size([1, 9, 256])
            dec_query_embeds_all_hidden_states = outputs.dec_query_embeds_all_hidden_states #torch.Size([1, 6, 900, 256])
            init_topk_coords = outputs.init_topk_coords #torch.Size([1, 900, 4])
            dec_refine_coords_all_hidden_states = outputs.dec_refine_coords_all_hidden_states #torch.Size([1, 6, 900, 4])
        else:
            idx = 5 + int(output_attentions) + int(output_hidden_states)
            enc_text_features_states = outputs[idx]
            dec_query_embeds_all_hidden_states = outputs[2]
            init_topk_coords = outputs[1]
            dec_refine_coords_all_hidden_states = outputs[3]
        return enc_text_features_states, dec_query_embeds_all_hidden_states, init_topk_coords, dec_refine_coords_all_hidden_states
    

    def _compute_predictions(
        self, enc_text_features_states, dec_query_embeds_all_hidden_states, init_topk_coords, dec_refine_coords_all_hidden_states, text_attention_mask
    ):
        outputs_hidden_classes, outputs_hidden_coords = [], []
        num_levels = dec_query_embeds_all_hidden_states.shape[1]

        for level in range(num_levels):
            refine_coords = init_topk_coords if level == 0 else dec_refine_coords_all_hidden_states[:, level - 1]
            refine_coords = torch.special.logit(refine_coords, eps=1e-5)

            class_logits = self.class_embed[level](
                vision_hidden_state=dec_query_embeds_all_hidden_states[:, level], #解码器的图像信息
                text_hidden_state=enc_text_features_states, #编码器的文本信息
                text_attention_mask=text_attention_mask.bool(),
            )
            outputs_hidden_classes.append(class_logits) #->torch.Size([1, 900, 256])

            delta_bbox = self.bbox_embed[level](dec_query_embeds_all_hidden_states[:, level]) #解码器的图像信息预测坐标偏差 torch.Size([1, 6, 900, 256]) ->torch.Size([1, 900, 4])
...
# outputs_classes, outputs_coords, outputs_hidden_classes, outputs_hidden_coords, labels
    ...
    def forward(
        ...
        token_type_ids: torch.LongTensor = None, #torch.Size([1, 9]) 哪一批
        attention_mask: torch.LongTensor = None, #torch.Size([1, 9]) 关注哪
        ...
        """ 解析输出的信息 """
        enc_text_features_states, dec_query_embeds_all_hidden_states, init_topk_coords, dec_refine_coords_all_hidden_states = self._process_outputs(outputs, output_attentions, output_hidden_states, return_dict) #->torch.Size([1, 9, 256]) torch.Size([1, 6, 900, 256])  torch.Size([1, 900, 4]) torch.Size([1, 6, 900, 4])


        outputs_classes, outputs_coords, outputs_hidden_classes, outputs_hidden_coords = self._compute_predictions( #加入中间层的信息
            enc_text_features_states, dec_query_embeds_all_hidden_states, init_topk_coords, dec_refine_coords_all_hidden_states, text_attention_mask
        )
        ...
        if return_dict:
            return GroundingDinoObjectDetectionOutput(
                ...
                outputs_classes=outputs_classes, #最重要的两个信息
                outputs_coords=outputs_coords, #最重要的两个信息
                # dec_refine_coords_all_hidden_states提供坐标  dec_query_embeds_all_hidden_states提供偏差
                dec_query_embeds_states=outputs.dec_query_embeds_states,
                dec_query_embeds_all_hidden_states=outputs.dec_query_embeds_all_hidden_states,
                dec_refine_coords_all_hidden_states=outputs.dec_refine_coords_all_hidden_states,
                dec_all_attentions=outputs.dec_all_attentions,
                
                enc_vision_features_states=outputs.enc_vision_features_states,
                enc_text_features_states=outputs.enc_text_features_states,
                enc_vision_all_hidden_states=outputs.enc_vision_all_hidden_states,
                enc_text_all_hidden_states=outputs.enc_text_all_hidden_states,
                enc_all_attentions=outputs.enc_all_attentions,
