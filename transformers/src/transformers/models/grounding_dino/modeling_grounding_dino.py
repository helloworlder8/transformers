...
class GroundingDinoEncoder(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        return GroundingDinoEncoderOutput(
            encoder_vision_features_states=vision_features, #torch.Size([1, 17821, 256])
            encoder_text_features_states=text_features, #torch.Size([1, 9, 256])

            encoder_vision_hidden_states=vision_hidden_states, #none
            encoder_text_hidden_states=text_hidden_states, #none
            encoder_attentions=encoder_attentions, #none
        )
...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    ...
    def encoder_query_proposals(self, enc_output, vision_attention_masks, vision_map_shapes):
    ...
    def forward(
        ...
        # Step 4: 提取视觉特征
        vision_features, vision_attention_masks, vision_posi_embed = self._extract_vision_features(pixel_values, pixel_mask) #torch.Size([1, 256, 100, 134]) torch.Size([1, 256, 50, 67]) torch.Size([1, 256, 25, 34]) torch.Size([1, 256, 13, 17]) 总之就是图像处理成金字塔  
        # Step 5: 视觉特征进行处理
        (
            batch_size, #1
            vision_features , #torch.Size([1, 17821, 256])
        ...
        (
            ...
            enc_outputs_class,
            enc_outputs_coord,
        ) = self._prepare_decoder_inputs(query_embeds, encoder_outputs, vision_attention_masks , vision_map_shapes, text_attention_mask, batch_size) #torch.Size([1, 17821]) torch.Size([4, 2]) 1 torch.Size([900, 256]) torch.Size([1, 9])
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel): #理解为整个项目
    ...
    def _process_outputs(self, return_dict, outputs, output_attentions, output_hidden_states):
        if return_dict:
...
# outputs_classes, outputs_coords, outputs_hidden_classes, outputs_hidden_coords, labels
    ...
    def forward(
        ...
        encoder_text_features_states, decoder_query_embeds_hidden_states, init_topk_coords, decoder_refine_coords_hidden_states = self._process_outputs(return_dict, outputs, output_attentions, output_hidden_states)
