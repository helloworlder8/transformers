...
class GroundingDinoProcessor(ProcessorMixin): #简单理解为前处理和后处理
    r"""
    ...
    def post_process(
        ...
        results = []
        for idx, (probs_i, scores_i, coords_i) in enumerate(zip(probs, scores, outputs_coords)): #torch.Size([900, 256]) torch.Size([900]) torch.Size([900, 4])
            valid_scores = scores_i[scores_i > box_threshold] #900->2 得分
