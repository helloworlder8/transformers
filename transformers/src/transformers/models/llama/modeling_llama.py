...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask, #torch.Size([1, 597])
            position_ids=position_ids, #torch.Size([1, 597])
            past_key_values=past_key_values, #DynamicCache()
            inputs_embeds=inputs_embeds, #torch.Size([1, 597, 4096])
            use_cache=use_cache, #True
            output_attentions=output_attentions, #false
            output_hidden_states=output_hidden_states, #false
            return_dict=return_dict, #true
            cache_position=cache_position, #torch.Size([597])
            **kwargs,
        )
