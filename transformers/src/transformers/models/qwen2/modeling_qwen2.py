...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        ) # torch.Size([1, 3343]) torch.Size([1, 3343, 896]) torch.Size([3343]) DynamicCache() False
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states, #torch.Size([1, 3343, 896])
                    attention_mask=causal_mask, #none
                    position_ids=position_ids, #torch.Size([1, 3343])
                    past_key_value=past_key_values, #DynamicCache()
                    output_attentions=output_attentions, #false
                    use_cache=use_cache, #true
                    cache_position=cache_position, #torch.Size([3343])
                    position_embeddings=position_embeddings, #(torch.Size([1, 3343, 64]),torch.Size([1, 3343, 64]))
                ) 
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        outputs = self.model(
            ...
            inputs_embeds=inputs_embeds, #torch.Size([1, 3343, 896])
            use_cache=use_cache, #true
            output_attentions=output_attentions, #false
        ...
        hidden_states = outputs[0] #->torch.Size([1, 1, 896])
        # Only compute necessary logits, and do not upcast them to float if we are not computing the loss
        logits = self.lm_head(hidden_states[:, -num_logits_to_keep:, :]) #torch.Size([1, 1, 896])->torch.Size([1, 1, 151679])
