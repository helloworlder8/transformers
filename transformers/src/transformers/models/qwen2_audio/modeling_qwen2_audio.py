...
class Qwen2AudioMultiModalProjector(nn.Module):
    ...
    def forward(self, audio_features):
        hidden_states = self.linear(audio_features) #torch.Size([2, 750, 1280]) 两句话 每句话最长750个单词 每个单词编码成1280的向量 torch.Size([2, 750, 4096])
...
class Qwen2AudioForConditionalGeneration(Qwen2AudioPreTrainedModel, GenerationMixin):
    ...
    def _merge_input_ids_with_audio_features(
        ...
        """
        Merge input IDs with audio features for multimodal processing.
        """
    ...
    def _determine_padding_side(self, attention_mask, batch_size):
        """
        Determine whether the padding is on the left or right side.
        """
        left_padding = torch.any(attention_mask[:, 0] == 0) #false
        right_padding = torch.any(attention_mask[:, -1] == 0) #false

        if batch_size > 1:
            if left_padding and not right_padding:
                return True
            elif not left_padding and right_padding:
                return False
            elif not left_padding and not right_padding:
                return self.padding_side == "left"
            else:
                raise ValueError(f"Invalid attention mask: both sides have zero padding. {attention_mask}")
        return left_padding

    def _calculate_audio_overwrite_mask(self, batch_size, max_token_num, token_placeholder_num, attention_mask, left_padding):
        """
        Calculate the mask for audio tokens to overwrite positions.
        """
        seq_indices = torch.arange(max_token_num, device=attention_mask.device).unsqueeze(0).expand(batch_size, -1)
        effective_token_count = token_placeholder_num.sum(-1) - (attention_mask == 0).long().sum(-1)

        if left_padding:
            mask = (max_token_num - seq_indices) <= effective_token_count[:, None]
        else:
            mask = seq_indices < effective_token_count[:, None]
    ...
    def forward(
        ...
        output_attentions = output_attentions or self.config.output_attentions #false
        output_hidden_states = output_hidden_states or self.config.output_hidden_states  #false
        return_dict = return_dict or self.config.use_return_dict  #true

        # Move input features and attention mask to target device
        if input_features is not None: #音频信号存入gpu处理
            input_features, feature_attention_mask = input_features.to(self.audio_tower.device), feature_attention_mask.to(self.audio_tower.device)

        # Extract or prepare input embeddings
        if inputs_embeds is None:
            inputs_embeds = self.get_input_embeddings()(input_ids) #-》torch.Size([1, 58, 4096])

            # Handle merging of text and audio features
            if input_features is not None and input_ids.shape[1] != 1: #有音频也有文字
                audio_features, audio_output_lengths = self._process_audio_features(input_features, feature_attention_mask) #torch.Size([2, 128, 3000])-》torch.Size([2, 750, 4096])
                inputs_embeds, attention_mask, labels, position_ids, _ = self._merge_input_ids_with_audio_features(
                    audio_features, audio_output_lengths, inputs_embeds, input_ids, attention_mask, labels
                ) #torch.Size([2, 750, 4096])   tensor([225, 318]) torch.Size([1, 58, 4096]) torch.Size([1, 58]) torch.Size([1, 58]) none  -》torch.Size([1, 599, 4096]) torch.Size([1, 599]) none torch.Size([1, 599])
        ...
        loss = self._compute_loss(logits, labels, attention_mask) if labels is not None else None

        # Return results based on return_dict
        if not return_dict:
        ...
        return Qwen2AudioCausalLMOutputWithPast(
            loss=loss, #none
            logits=logits, #torch.Size([1, 599, 156032])
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states, #none
            attentions=outputs.attentions, #none
            attention_mask=attention_mask, #torch.Size([1, 599])
        )

    # Helper methods
    def _process_audio_features(self, input_features, feature_attention_mask):
        """
        Process and prepare audio features for merging.
        """
        ...
        return self.multi_modal_projector(audio_outputs.last_hidden_state),audio_output_lengths #torch.Size([2, 750, 1280])
    ...
    def _compute_loss(self, logits, labels, attention_mask):
        ...
        if attention_mask is not None:
            ...
            shift_logits, shift_labels = shift_logits[mask != 0].contiguous(), shift_labels[mask != 0].contiguous()

        loss_fct = nn.CrossEntropyLoss()
        return loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1).to(shift_logits.device))
