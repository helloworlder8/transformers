...
class Sa2VAChatModel(PreTrainedModel):
    ...
    def extract_feature(self, pixel_values):
        if self.select_layer == -1:
            vit_embeds = self.vision_model(
                pixel_values=pixel_values, #torch.Size([13, 3, 448, 448])
                output_hidden_states=False,
                return_dict=True).last_hidden_state #->torch.Size([13, 1025, 1024])
        else:
            vit_embeds = self.vision_model(
        ...
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], h, w, -1) #torch.Size([13, 32, 32, 1024])
        vit_embeds = self.pixel_shuffle(vit_embeds, scale_factor=self.downsample_ratio) #->torch.Size([13, 16, 16, 4096])
        vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], -1, vit_embeds.shape[-1]) #->torch.Size([13, 256, 4096]) #batch num_patch dim
        vit_embeds = self.mlp1(vit_embeds)
        return vit_embeds #->torch.Size([13, 256, 896]) batch num_patch dim
    ...
    def generate(
        ...
        if pixel_values is not None: #torch.Size([13, 3, 448, 448])
            if visual_features is not None: #none
                ...
                vit_embeds = self.extract_feature(pixel_values.to(device)) #torch.Size([13, 256, 896]) batch num_patch dim
            image_flags = torch.sum(pixel_values, dim=(1, 2, 3)) != 0 #true*13
            image_flags = image_flags.long() #torch.Size([13]) 1
            vit_embeds = vit_embeds[image_flags == 1]

            input_embeds = self.language_model.get_input_embeddings()(input_ids.to(device)) #torch.Size([1, 3343])->torch.Size([1, 3343, 896])
            B, N, C = input_embeds.shape #torch.Size([1, 3343, 896]) batch seq_len dim
            input_embeds = input_embeds.reshape(B * N, C)
            ...
            input_ids = input_ids.reshape(B * N) #torch.Size([3343])
            selected = (input_ids == self.img_context_token_id) #3343里面有3328是图像
        ...
        outputs = self.language_model.generate(
            inputs_embeds=input_embeds, #torch.Size([1, 3343, 896]) #有文字信息也有图片信息
            attention_mask=attention_mask.to(device), #torch.Size([1, 3343])
    ...
    def preparing_for_generation(self, tokenizer, max_new_tokens=2048, torch_dtype=torch.bfloat16):
        ...
        if not hasattr(self, 'tokenizer'):
            self.tokenizer = tokenizer #使用外部传来的tokenizer
        self.bot_name = 'BOT'
        stop_words = []
        stop_words += self.template.get('STOP_WORDS', []) #['<|im_end|>', '<|endoftext|>']
        stop_criteria = get_stop_criteria(
            tokenizer=self.tokenizer, stop_words=stop_words)
        self.stop_criteria = stop_criteria #class
        ...
        ) #{'max_new_tokens': 2048, 'do_sample': False, 'eos_token_id': 151645, 'pad_token_id': 151643}

        self.gen_config = GenerationConfig(**default_generation_kwargs)
        self.init_prediction_config = True #flag 初始化生成配置
        ...
        self.num_patch = int((self.image_size // patch_size) ** 2 * (self.downsample_ratio ** 2))
        ...
        self.img_context_token_id = img_context_token_id #哪一个是img token
        self.seg_token_idx = tokenizer.convert_tokens_to_ids('[SEG]') #哪一个是seg token
        return
    ...
    def predict_forward(
        ...
        if video is not None:
            ...
            num_image_tokens = self.num_patch
            num_frames = 5

            input_dict['vp_overall_mask'] = None
        else:
            ori_image_size = image.size #(640, 480)

            # prepare grounding images
            g_image = np.array(image)  # for grounding (480, 640, 3)
            g_image = self.extra_image_processor.apply_image(g_image) #(1024, 1024, 3)
            g_pixel_values = torch.from_numpy(g_image).permute(2, 0, 1).contiguous().to(self.torch_dtype) #torch.Size([3, 1024, 1024])
            extra_pixel_values = [g_pixel_values]
            g_pixel_values = torch.stack([
                self.grounding_encoder.preprocess_image(pixel) for pixel in extra_pixel_values
            ]).to(self.torch_dtype) #torch.Size([1, 3, 1024, 1024])

            images = dynamic_preprocess(image, self.min_dynamic_patch,
                                        self.max_dynamic_patch,
                                        self.image_size, self.use_thumbnail) #xy640x480 ->xy 13*448x448
            ...
            pixel_values = [self.transformer(image) for image in images] #[13*torch.Size([3, 448, 448])]
            pixel_values = torch.stack(pixel_values).to(self.torch_dtype) #torch.Size([13, 3, 448, 448])
            num_image_tokens = pixel_values.shape[0] * self.num_patch
            num_frames = 1
        ...
        mm_inputs = {
            'pixel_values': input_dict['pixel_values'], #torch.Size([13, 3, 448, 448])
            'input_ids': ids, #torch.Size([1, 3343])
            'attention_mask': attention_mask, #torch.Size([1, 3343])
            'position_ids': None,
            'past_key_values': None,
            'labels': None,
            'prompt_masks': mask_prompts, #none
        ...
        generate_output = self.generate(
            **mm_inputs,
            ...
            streamer=None,
            bos_token_id=self.tokenizer.bos_token_id, #None
            stopping_criteria=self.stop_criteria, #[class,class]
            output_hidden_states=True,
        ...
        hidden_states = generate_output.all_hidden_states
        ...
        for seg_hidden_states in all_seg_hidden_states:
            ...
            pred_masks = self.grounding_encoder.language_embd_inference(sam_states, [seg_hidden_states] * num_frames) #抽出seg hidden
            w, h = ori_image_size
