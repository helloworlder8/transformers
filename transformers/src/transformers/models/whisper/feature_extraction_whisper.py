...
class WhisperFeatureExtractor(SequenceFeatureExtractor): #耳语特征提取器，来自openai
    ...
    def _torch_extract_fbank_features(self, waveform: np.array, device: str = "cpu") -> np.ndarray:
        ...
        window = torch.hann_window(self.n_fft).to(device) #torch.Size([400])
        ...
        mel_spec = torch.matmul(mel_filters.T, magnitudes) #torch.Size([201, 80]) torch.Size([1, 201, 3000])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """ 判断是否为 batch 输入，转为 float32 格式 """
        if is_batched:
            raw_speech = [np.asarray([speech], dtype=np.float32).T for speech in raw_speech]
        elif not is_batched and not isinstance(raw_speech, np.ndarray):
            raw_speech = np.asarray(raw_speech, dtype=np.float32)
        elif isinstance(raw_speech, np.ndarray) and raw_speech.dtype is np.dtype(np.float64):
            raw_speech = raw_speech.astype(np.float32) #->(93680,)

        # 强制 batch 结构
        if not is_batched:
            raw_speech = [np.asarray([raw_speech]).T] #[(93680, 1)]
        ...
        # 6. 填充
        padded_inputs = self.pad( #-》(1, 480000, 1)
            batched_speech, #(93680, 1)
            padding=padding, #'max_length'
            max_length=max_length or self.n_samples, #480000
            truncation=truncation, #true
            pad_to_multiple_of=pad_to_multiple_of, #none
            return_attention_mask=return_attention_mask or do_normalize, #none
        )
        ...
        input_features = extract_fbank_features(input_features[0], device) #(1, 480000)-》(1, 80, 3000)
