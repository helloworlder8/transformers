...
class ProcessorMixin(PushToHubMixin):
    ...
    def get_processor_dict(
        ...
        if os.path.isdir(model_name_or_path):
            processor_file = os.path.join(model_name_or_path, PROCESSOR_NAME)
            chat_template_file = os.path.join(model_name_or_path, "chat_template.json")

        if os.path.isfile(model_name_or_path):
            resolved_processor_file = model_name_or_path
            # cant't load chat-template when given a file as model_name_or_path
            resolved_chat_template_file = None
            is_local = True
        elif is_remote_url(model_name_or_path):
            processor_file = model_name_or_path
            resolved_processor_file = download_url(model_name_or_path)
            # can't load chat-template when given a file url as model_name_or_path
            resolved_chat_template_file = None
        else:
            ...
            try:
                ...
                raise EnvironmentError(
                    f"Can't load processor for '{model_name_or_path}'. If you were trying to load"
    ...
    def _merge_kwargs(
        ...
        **kwargs,
        ...
        # Initialize default and output dictionaries
        modality_keys = ["text_kwargs", "images_kwargs", "audio_kwargs", "videos_kwargs", "common_kwargs"]
        output_kwargs = {modality: {} for modality in modality_keys}
        default_kwargs = {
            modality: ModelProcessorKwargs._defaults.get(modality, {}).copy() for modality in modality_keys
        }

        used_keys = set()

        # Update defaults with tokenizer initialization arguments
        for modality, defaults in default_kwargs.items():
            for key in ModelProcessorKwargs.__annotations__[modality].__annotations__.keys():
                if key in tokenizer_init_kwargs:
                    defaults[key] = (
                        getattr(self.tokenizer, key, tokenizer_init_kwargs[key])
                    )

        # Merge default_kwargs into output_kwargs
        output_kwargs.update(default_kwargs)

        # Process passed kwargs
        for modality, defaults in output_kwargs.items():
            for key in ModelProcessorKwargs.__annotations__[modality].__annotations__.keys():
                if modality in kwargs:  # Check nested modality dict
                    kwarg_value = kwargs[modality].pop(key, None)
                else:  # Check flat dictionary
                    kwarg_value = kwargs.get(key, None)

                if kwarg_value is not None:
                    if key in used_keys:
                        raise ValueError(
                            f"Keyword argument {key} was passed twice: in {modality} dictionary and as a flat kwarg."
                        )
                    defaults[key] = kwarg_value
                    used_keys.add(key)

        # Handle any unmatched nested dictionaries in kwargs
        for modality, subdict in kwargs.items():
        ...
        # Handle unmatched flat kwargs
        for key, value in kwargs.items():
            if key not in used_keys:
                if key in ModelProcessorKwargs.__annotations__["common_kwargs"].__annotations__.keys():
                    output_kwargs["common_kwargs"][key] = value
                else:
                    logger.warning_once(
                        f"Keyword argument `{key}` is not a valid argument for this processor and will be ignored."
                    )

        # Apply common kwargs to all modality-specific kwargs
        for modality in output_kwargs:
            output_kwargs[modality].update(output_kwargs["common_kwargs"])
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        args = cls._get_arguments_from_pretrained(model_name_or_path, **kwargs)
        processor_dict, kwargs = cls.get_processor_dict(model_name_or_path, **kwargs)
    ...
    @classmethod
    def _get_arguments_from_pretrained(cls, model_name_or_path, **kwargs):
        args = []
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        return self.tokenizer.apply_chat_template( #处理器中的tokenizer中调用这个函数
...
def _validate_images_text_input_order(images, text):
    ...
    def _is_valid(input, validator):
        return validator(input) or input is None #有输入就用函数来验证输入
