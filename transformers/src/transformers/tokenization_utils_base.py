...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def _render_conversations(
        ...
        **kwargs,
        ...
        参数:
            conversations (list): 对话内容，嵌套 list + dict 结构。
            chat_template (str): Jinja 模板字符串。
            special_tokens_map (dict): 特殊符号映射，用于模板渲染。
            tools (optional): 工具信息（默认为 None）。
            documents (optional): 文档信息（默认为 None）。
            return_assistant_tokens_mask (bool): 是否返回 assistant token 的掩码。
            continue_final_message (bool): 是否继续处理最后一条消息。
            add_generation_prompt (bool): 是否添加生成提示。
            is_batched (bool): 是否为 batch 输入（多条会话）。
            **kwargs: 其他传入 `render_jinja_template` 的参数。

        返回:
            rendered_chat: 渲染后的对话字符串或列表。
            generation_indices: 渲染时标记的生成位置索引。
        """
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        chat_template = self._get_chat_template(chat_template, tools) #none none ->str

        """ 对话应用模板 """
        rendered_chat, generation_indices = self._render_conversations(
            conversations=conversations,
            ...
            return_assistant_tokens_mask=return_assistant_tokens_mask, #false
            continue_final_message=continue_final_message, #false
    ...
    def _get_chat_template(self, chat_template: Optional[str] = None, tools: Optional[List[Dict]] = None) -> str:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 处理输入文本
        if text is not None: #[str]
            if not self._in_target_context_manager:
                self._switch_to_input_mode()
            encodings = self._call_one(text=text, text_pair=text_pair, **all_kwargs)#[str] none {}
    ...
    def decode(
        ...
        **kwargs,
        ...
        return self._decode( #class
            token_ids=token_ids, #[len221]
            skip_special_tokens=skip_special_tokens, #true
            clean_up_tokenization_spaces=clean_up_tokenization_spaces, #none
            **kwargs, #{}
        )
